...
## 2026-03-21 circle visible-range cache in resize loop round

QEMU profile: `python scripts/code_perf_check.py --profile cortex-m3`

| Test | Before (ms) | After (ms) | Delta |
|------|-------------|------------|-------|
| MASK_IMAGE_CIRCLE | 2.483 | 2.430 | -2.1% |
| MASK_IMAGE_CIRCLE_QUARTER | 0.783 | 0.773 | -1.3% |
| MASK_IMAGE_CIRCLE_DOUBLE | 2.400 | 2.339 | -2.5% |
| MASK_IMAGE_ROUND_RECT | 2.132 | 2.135 | +0.1% |
| MASK_IMAGE_ROUND_RECT_QUARTER | 0.707 | 0.708 | +0.1% |
| MASK_IMAGE_ROUND_RECT_DOUBLE | 2.096 | 2.098 | +0.1% |
| IMAGE_RESIZE_565 | 1.651 | 1.651 | 0.0% |
| IMAGE_RESIZE_565_8 | 1.654 | 1.654 | 0.0% |
| MASK_IMAGE_NO_MASK | 1.654 | 1.654 | 0.0% |
| RECTANGLE_FILL | 0.395 | 0.395 | 0.0% |

- `src/image/egui_image_std.c` now keeps the circle visible-span lookup inside the `RGB565` resize draw loop, so the hot `MASK_IMAGE_CIRCLE*` path reuses a local per-row visible-half cache instead of bouncing back through `mask_get_row_visible_range()` for every partial row.
- The new visible-range fast path only activates after `mask_get_row_range()` has already produced a valid cached circle row, which keeps the generic mask API unchanged while matching the row-by-row scanline style used in `ref/scgui`.
- Validation: `python scripts/code_perf_check.py --profile cortex-m3`, `python scripts/code_runtime_check.py --app HelloPerformance --timeout 120 --keep-screenshots`, screenshot diff vs `runtime_check_output/HelloPerformance_baseline_pre_rectfill_opt_20260320/default` is `60/60` pixel-identical, and `make clean && make all APP=HelloUnitTest PORT=pc_test && output\main.exe` stays `554/554 passed`.

## 2026-03-21 circle local row-range in resize loop round

QEMU profile: `python scripts/code_perf_check.py --profile cortex-m3`

| Test | Before (ms) | After (ms) | Delta |
|------|-------------|------------|-------|
| MASK_IMAGE_CIRCLE | 2.430 | 2.253 | -7.3% |
| MASK_IMAGE_CIRCLE_QUARTER | 0.773 | 0.731 | -5.4% |
| MASK_IMAGE_CIRCLE_DOUBLE | 2.339 | 2.163 | -7.5% |
| MASK_IMAGE_ROUND_RECT | 2.135 | 2.148 | +0.6% |
| MASK_IMAGE_ROUND_RECT_QUARTER | 0.708 | 0.711 | +0.4% |
| MASK_IMAGE_ROUND_RECT_DOUBLE | 2.098 | 2.111 | +0.6% |
| IMAGE_RESIZE_565 | 1.651 | 1.651 | 0.0% |
| IMAGE_RESIZE_565_8 | 1.654 | 1.654 | 0.0% |
| MASK_IMAGE_NO_MASK | 1.654 | 1.654 | 0.0% |
| RECTANGLE_FILL | 0.395 | 0.395 | 0.0% |

- `src/image/egui_image_std.c` now computes the circle row's opaque band directly inside the `RGB565` resize loop, with local row-index and opaque-boundary caches, so the hot `MASK_IMAGE_CIRCLE*` path no longer dispatches through `mask_get_row_range()` on every scanline.
- Combined with the previous visible-range cache, the resize fast path now mirrors the scanline-local style seen in `ref/scgui`: first derive row geometry once, then blend left edge, opaque middle, and right edge without re-entering the generic mask API.
- Validation: `python scripts/code_perf_check.py --profile cortex-m3`, `python scripts/code_runtime_check.py --app HelloPerformance --timeout 120 --keep-screenshots`, screenshot diff vs `runtime_check_output/HelloPerformance_baseline_pre_rectfill_opt_20260320/default` is `60/60` pixel-identical, and `make clean && make all APP=HelloUnitTest PORT=pc_test && output\main.exe` stays `554/554 passed`.
...
## 2026-03-21 round-rect masked RGB565 resize edge fast path round

QEMU profile: `python scripts/code_perf_check.py --profile cortex-m3`

| Test | Before (ms) | After (ms) | Delta |
|------|-------------|------------|-------|
| MASK_IMAGE_ROUND_RECT | 2.152 | 1.914 | -11.1% |
| MASK_IMAGE_ROUND_RECT_QUARTER | 0.712 | 0.648 | -9.0% |
| MASK_IMAGE_ROUND_RECT_DOUBLE | 2.115 | 1.918 | -9.3% |
| MASK_IMAGE_CIRCLE | 1.927 | 1.932 | +0.3% |
| MASK_IMAGE_CIRCLE_QUARTER | 0.670 | 0.671 | +0.1% |
| MASK_IMAGE_CIRCLE_DOUBLE | 1.739 | 1.744 | +0.3% |
| RECTANGLE_FILL | 0.395 | 0.395 | 0.0% |
...
## 2026-03-21 round-rect neighbor-row boundary cache round

QEMU profile: `python scripts/code_perf_check.py --profile cortex-m3`

| Test | Before (ms) | After (ms) | Delta |
|------|-------------|------------|-------|
| MASK_IMAGE_ROUND_RECT | 1.863 | 1.859 | -0.2% |
| MASK_IMAGE_ROUND_RECT_QUARTER | 0.636 | 0.634 | -0.3% |
| MASK_IMAGE_ROUND_RECT_DOUBLE | 1.856 | 1.855 | -0.1% |
| MASK_IMAGE_CIRCLE | 1.932 | 1.932 | 0.0% |
| MASK_IMAGE_CIRCLE_QUARTER | 0.671 | 0.671 | 0.0% |
| MASK_IMAGE_CIRCLE_DOUBLE | 1.744 | 1.744 | 0.0% |
| RECTANGLE_FILL | 0.395 | 0.395 | 0.0% |
