...
## Step 0: Gap discovery + design brief
...
Each new widget must complete at least 10 recursive iteration cycles before the flow can be closed.
...
- gate requires enough real iteration diversity:
  - `unique_change_cycles >= max(3, min_cycles/2)` (default 10 cycles => >=5)
  - `visual_delta_cycles >= max(3, min_cycles/2)` (default 10 cycles => >=5)
...
5. Recursive iteration gate passes (`>=10` cycles and latest PASS).
...
- Recursive iteration gate must pass (`min-iterations >= 10`, latest cycle PASS).
