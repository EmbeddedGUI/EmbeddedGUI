...
EmbeddedGUI 资源生成管线：将图片/SVG、MP4 帧和 TTF 字体转换为 C 数组或外部 bin 文件。

## 资源配置文件

主配置位置：`example/{APP}/resource/src/app_resource_config.json`

生成器实际会读取“有效配置”：

- 用户配置：`resource/src/app_resource_config.json`
- Designer 配置：`resource/src/.designer/app_resource_config_designer.json`，兼容旧位置 `resource/src/app_resource_config_designer.json`
- 如果同时存在 Designer 配置和用户配置，会合并后写入 `resource/src/.designer/.app_resource_config_merged.json`
...
| 字段 | 说明 | 可选值 |
|------|------|--------|
| `file` | 源图片文件名（在 `resource/src/` 下） | PNG/BMP/SVG 等 `img2c.py` 可读取格式 |
| `name` | 资源 C 名称后缀（可选，默认取文件名） | C 标识符友好的名称 |
| `format` | 像素格式；`svg` 表示保留 SVG 源数据 | `rgb565`, `rgb32`, `gray8`, `alpha`, `svg` |
| `alpha` | Alpha 位深 | `0`, `1`, `2`, `4`, `8` |
| `external` | 存储方式 | `0`=内部C数组, `1`=外部bin文件 |
| `swap` | RGB565 字节交换 | `0`, `1` |
| `dim` | 缩放尺寸（可选） | `"64,64"` |
| `rot` | 旋转角度（可选） | 角度数值 |
| `bg` | Alpha 合成背景色（可选） | `#000000` |
| `compress` | 图片压缩（可选） | `none`, `rle`, `qoi` |

SVG 规则：`format: "svg"` 会生成 `egui_svg_source_t` 原始 SVG 资源；如果把 `.svg` 当普通图片栅格化，必须同时配置 `dim`。`qoi` 仅适合 `rgb565` / `rgb32`，用于 `gray8` / `alpha` 时生成器会退回非压缩变体。
...
| `name` | 资源 C 名称后缀（可选，默认取文件名） | C 标识符友好的名称 |
| `pixelsize` | 字号（像素高度） | 整数；`all` 会生成 4-32 |
| `fontbitsize` | 字形位深 | `1`, `2`, `4`, `8` |
| `external` | 存储方式 | `0`=内部, `1`=外部 |
| `text` | 字符集文件 | 包含所有需要渲染的字符 |
| `weight` | 可变字体字重（可选） | 如 `400`, `500`, `700` |
| `compress` | 字体位图压缩（可选，仅 4bpp 有效） | `none`, `rle4`, `rle4_xor` |

字符集文件（`text`）为 UTF-8 纯文本，包含所有需要的字符。支持 HTML 实体如 `&#x2103;`（℃）。

注意：当前 `app_resource_generate.py` 不会把单个 `text` 字段里的逗号拆成多个文件路径。需要合并多个 text 文件时，优先先合并成一个 text 文件；或者为同一字体写多条 `font` 配置，让生成器按相同 `name/pixelsize/fontbitsize/external/compress` 合并。
...
配置中也支持 `mp4` 段。生成器会用 `ffmpeg` 抽帧，把帧按图片资源管线生成，并额外输出 `app_egui_resource_mp4_{name}.h` 帧数组头文件。常用字段：

| 字段 | 说明 |
|------|------|
| `file` | 源视频文件，相对 `resource/src/` |
| `name` | 帧组名称 |
| `fps` | 抽帧帧率，默认 `10` |
| `width` / `height` | 输出帧尺寸，`0` 表示不缩放 |
| `format` / `alpha` / `external` / `compress` | 同图片资源字段 |
...
每次在代码中新增中文文本或图标字符后，都应重新运行脚本，然后执行 `make resource_refresh APP={APP}`。
...
| 问题 | 原因 | 修复 |
|------|------|------|
| `undefined reference to 'egui_res_image_xxx'` | 资源未生成或名称不匹配 | 检查 config.json 中 file 字段，运行 `make resource_refresh APP={APP}` |
...
| 生成跳过（使用缓存） | `app_egui_resource_merge.bin` 已存在，或生成的 `app_egui_resource_generate.h/.c` 已存在 | 用 `make resource_refresh APP={APP}` 强制重建 |
...
`EGUI_CODE_SRC` 在当前 Makefile 中表示“源码目录”，不要把 `$(wildcard .../*.c)` 放进这里。若确实要登记单个源文件，使用 `EGUI_CODE_SRC_FILES`。
