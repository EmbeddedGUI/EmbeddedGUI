...
- `EGUI_CONFIG_SCREEN_WIDTH` 已从 app override 移除，直接继承库默认 `240`。
...
- 结果：
  - 删除 `EGUI_CONFIG_SCREEN_WIDTH` app override 后，`read_screen_size()` 仍返回 `(240, 240)`
  ...
  - `scripts/perf_analysis/code_perf_check.py` 现在按维度读取 `EGUI_CONFIG_SCREEN_WIDTH / HEIGHT`，缺省时分别回退到库默认值
...
- `uicode_disp0.c` 和 `egui_view_test_performance.c` 继续通过 `EGUI_CONFIG_SCREEN_WIDTH` 使用同一套 `240px` 几何；去掉 app 侧重复定义不会改变编译后的 benchmark harness。
...
- `egui_view_test_performance.c` 的全文字、整屏图片、mask、gradient、circle 等场景都按 `SCREEN_HEIGHT` 推导几何。
...
- `EGUI_CONFIG_SCREEN_WIDTH` 已回退到库默认 `240`
- `EGUI_CONFIG_SCREEN_HEIGHT`
- `EGUI_CONFIG_MAX_FPS`
- `EGUI_CONFIG_CIRCLE_SUPPORT_RADIUS_BASIC_RANGE`
- `EGUI_CONFIG_DEBUG_LOG_LEVEL`

本轮已确认 `EGUI_CONFIG_SCREEN_WIDTH` 可以直接继承库默认值，不再需要 app 侧宏管理。剩余 4 项仍不应按“`<100B static RAM` 且 `perf <5%` 就默认打开”的规则处理；它们定义的是 HelloPerformance benchmark 的几何、节奏、圆形 coverage 和报告输出环境，去掉这些显式定义，改变的是 benchmark harness，而不是仅仅减少宏维护成本。
