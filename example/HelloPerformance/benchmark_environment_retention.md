...
- 结果：
  ...
  - `scripts/code_perf_check.py` 会直接从 `app_egui_config.h` 读取 `EGUI_CONFIG_SCEEN_WIDTH / HEIGHT`
  - 报告生成依赖 `PERF_RESULT` / `PERF_SKIP` / `PERF_COMPLETE` 结构化日志

## 宏结论

### `EGUI_CONFIG_SCEEN_WIDTH` / `EGUI_CONFIG_SCEEN_HEIGHT`

- `uicode.c` 直接用这两个宏设置 benchmark root view 尺寸。
- `egui_view_test_performance.c` 的全文字、整屏图片、mask、gradient、circle 等场景都按 `SCEEN_WIDTH / HEIGHT` 推导几何。
- `scripts/code_perf_check.py` 也会从 `app_egui_config.h` 读取这两个值；它们不仅决定 workload 形状，也决定 perf 报告解释的是哪一套屏幕几何。
- 当前 app 显式固定 `240x240`，是为了把 clean perf 报告中的 `TEXT_ROTATE_BUFFERED 11.498ms`、`EXTERN_IMAGE_ROTATE_565_8 18.990ms`、`MASK_RECT_FILL_CIRCLE_DOUBLE 1.656ms` 等结果锚定到同一套 square harness。把 `HEIGHT` 回退到默认 `320` 会直接改变 benchmark 本身，而不是只减少一项 small-RAM 宏。
...
以上 5 项都不应按“`<100B static RAM` 且 `perf <5%` 就默认打开”的规则处理。它们定义的是 HelloPerformance benchmark 的几何、节奏、圆形 coverage 和报告输出环境；去掉这些显式定义，改变的是 benchmark harness，而不是仅仅减少宏维护成本。
