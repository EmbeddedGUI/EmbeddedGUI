...
- Current normal QEMU build: `text=2168304`, `data=52`, `bss=6252`, `static RAM=6304`.
...
- `g_text_transform_visible_tile_cache` still only stores the `8B` heap handle/capacity pair; the visible alpha8 tile itself remains heap-backed, and HelloPerformance now keeps the reuse ceiling at `3648B`: the old `4096B` option was not worth its tiny `64B` saving under the `<=500B => 5%` rule, while the latest `4160 -> 3648` trim saves `512B` and still keeps buffered rotated text within the `>500B => 10%` rule.
...
- The buffered rotated-text alpha8 fast-path ceiling now stays at `3648B`: trimming `4160 -> 3648` saves another `512B` in both buffered rotated-text hotspots (`TEXT_ROTATE_BUFFERED 5072 -> 4560`, `EXTERN_TEXT_ROTATE_BUFFERED 5342 -> 4830`) while keeping perf within the `>500B => 10%` rule (`+9.77%` / `+8.54%`).
...
| `EXTERN_TEXT_ROTATE_BUFFERED` | 4830 | `214 / 214` | External rotated-text visible alpha8 tile cache plus transient layout/tile scratch |
| `TEXT_ROTATE_BUFFERED` | 4560 | `151 / 151` | Rotated-text visible alpha8 tile cache plus transient layout/tile scratch |
...
- The latest buffered-text round trims `TEXT_ROTATE_BUFFERED` further from `5072B` to `4560B` by tightening the visible-alpha8 heap ceiling from `4160B` to `3648B`.
- The current next-largest verified non-codec hotspot is now `EXTERN_TEXT_ROTATE_BUFFERED` at `4830B`; the extra `270B` above the internal path is the remaining external glyph row-cache cost after delaying its allocation until fallback is actually needed.
- Even after cutting both buffered text scenes below `4.9KB`, the dominant heap headline is still controlled by compressed-image row caching rather than text.
