...
- Current normal QEMU build: `text=2165884`, `data=52`, `bss=2924`, `static RAM=2976`.
...
| `egui_canvas_draw_text_transform` | `src/core/egui_canvas_transform.c` | 760 | Current largest linked HelloPerformance frame | Layout/tile collectors now allocate by actual glyph and line count on transient heap, trimming this frame from `2920B` |
...
- The current largest linked HelloPerformance frame is `egui_canvas_draw_text_transform (760B)`; function-local `optimize("Os")` trims `egui_canvas_draw_line_hq` down to `288B`, and there is still no linked function frame `>= 1KB` in the final benchmark image.
...
- Removing the rejected whole-image cache hook keeps `egui_canvas_draw_image_transform` at `736B`, still below the current linked `760B` stack hotspot line.
