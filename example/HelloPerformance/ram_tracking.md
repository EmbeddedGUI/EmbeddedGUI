...
- Current normal QEMU build: `text=2168304`, `data=52`, `bss=4716`, `static RAM=4768`.
- Section split from `llvm-size -A`: `.bss=2152`, `._user_heap_stack=2564`.
- `HelloPerformance` now overrides the QEMU linker reserve with `__qemu_min_stack_size__=0x0a00`, so the reported static RAM headline reflects a 2.5KB stack budget instead of the port default 8KB.
...
- The HelloPerformance QEMU linker reserve is now `2.5KB` via `__qemu_min_stack_size__=0x0a00`; that still leaves more than `2x` headroom over the largest compiled frame (`1200B`) and about `2.6x` over the current active HelloPerformance max frame (`976B`).
