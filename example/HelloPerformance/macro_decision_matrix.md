...
| --- | --- | --- | --- | --- | --- |
| `EGUI_CONFIG_SCREEN_HEIGHT` | `240` | `320` | benchmark 环境 | 保留 | 当前 clean perf 锚定在 `240x240`；详见 `benchmark_environment_retention.md` |
| `EGUI_CONFIG_MAX_FPS` | `1` | `60` | benchmark 环境 | 保留 | benchmark 节奏锚点；详见 `benchmark_environment_retention.md` |
| `EGUI_CONFIG_PFB_WIDTH` | `48` | `(EGUI_CONFIG_SCREEN_WIDTH / 8)` | benchmark 环境 | 保留 | `48x16x1=1536B`；仅回退到 `30x30x1` 就是 `+264B`，且 `--pfb-matrix` 依赖此入口 |
| `EGUI_CONFIG_PFB_HEIGHT` | `16` | `(EGUI_CONFIG_SCREEN_HEIGHT / 8)` | benchmark 环境 | 保留 | 与 `PFB_WIDTH` 一起定义当前低 SRAM 基准几何；详见 `benchmark_override_retention.md` |
...
- `EGUI_CONFIG_SCREEN_WIDTH` 已回退到库默认 `240`，不再占用 app 侧宏管理。
