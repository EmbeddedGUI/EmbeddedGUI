...
| `EGUI_CONFIG_IMAGE_DECODE_OPAQUE_ALPHA_ROW_USE_ROW_CACHE` | 历史实验值 `1`（当前默认 `0`） | `0` | decode/heap 行为 | 保留 | 当前完整 A/B 无 perf/runtime/unit 差异，`1` 仅额外回收 `text -132B, bss -8B`；先记为条件实验值，详见 `codec_heap_override_retention.md` |
| `EGUI_CONFIG_IMAGE_QOI_CHECKPOINT_COUNT` | 历史实验值 `0`（当前默认 `2`） | `2` | decode/heap 行为 | 保留默认 | 当前完整 A/B 显示 `2 -> 0` 只回收 `text -276B, bss -8B`，但 `20` 个 QOI 场景回退 `+110% ~ +911%`；详见 `codec_heap_override_retention.md` |
...
| `EGUI_CONFIG_IMAGE_RLE_EXTERNAL_WINDOW_PERSISTENT_CACHE_ENABLE` | 历史实验值 `0`（当前默认 `1`） | `1` | decode/heap 行为 | 保留默认 | `2026-04-05` 当前 A/B：`1 -> 0` 可回收 `bss -1040B`，但会带来 `text +816B`，且 `IMAGE_TILED_RLE_565_0 +6.2%`、external RLE 主路径约 `+2.7% ~ +5.1%`；runtime/unit 等价，详见 `codec_heap_override_retention.md` |
...
| `EGUI_CONFIG_IMAGE_STD_ALPHA_OPAQUE_CACHE_SLOTS` | 历史实验值 `0`（当前默认 `4`） | `4` | static RAM | 保留默认 | `2026-04-05` 当前 recheck：`4 -> 0` 仍回收 `text -156B, bss -40B`，perf `ge10=0/le10=1`，但 runtime `4 vs 0` 仍有 `8` 帧真实像素差，且 `4 vs 4 repeat` 控制组全等；详见 `static_ram_override_retention.md` |
