...
#ifndef EGUI_PERF_CAPTURE_BENCHMARK_ONLY
...
/* QEMU -icount shift=0 is deterministic enough for single-pass timing.
 * Keeping one refresh sample avoids tripling the already long full-scene sweep. */
...
{
    // egui_screen_on(s_core) resumes the global refresh timer. Stop it again
    ...
    {
        egui_core_force_refresh(s_core);
        egui_polling_refresh_display(s_core);
    }
...
#else
    uint32_t start_time = egui_api_timer_get_current_core(s_core);
    for (int i = 0; i < TEST_REPEAT_COUNT; i++)
    {
        egui_core_force_refresh(s_core);
        egui_polling_refresh_display(s_core);
    ...
    // get time.
    start_time = egui_api_timer_get_current_core(s_core) - start_time;
    ...
    // egui_api_refresh_display(s_core) in pc will use too much time, so we calc time.
...
static int egui_view_test_performance_get_qemu_benchmark_test_mode(int scene_index)
{
    ...
    int scene_count = (int)(sizeof(s_qemu_benchmark_scenes) / sizeof(s_qemu_benchmark_scenes[0]));
...
#else
#if EGUI_PORT == EGUI_PORT_TYPE_QEMU
    for (int scene_index = 0;; scene_index++)
    {
        int test_mode = egui_view_test_performance_get_qemu_benchmark_test_mode(scene_index);
        ...
        egui_view_test_performance_with_test_mode(test_mode);
    }
#else
...
static int egui_view_test_performance_get_recording_test_mode(int action_index)
{
    ...
    int scene_count = (int)(sizeof(s_qemu_heap_measure_scenes) / sizeof(s_qemu_heap_measure_scenes[0]));

    if (action_index < 0 || action_index >= scene_count)
    {
        return -1;
    }
