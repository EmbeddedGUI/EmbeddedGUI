...
void uicode_toggle_theme(void)
{
    if (is_dark_theme)
    {
        egui_view_set_theme(EGUI_VIEW_OF(&root), &egui_theme_light);
        is_dark_theme = 0;
    }
    else
    {
        egui_view_set_theme(EGUI_VIEW_OF(&root), &egui_theme_dark);
        is_dark_theme = 1;
    }
...
void uicode_disp0_init(egui_core_t *core)
{
    ...
    egui_view_group_init_with_params(EGUI_VIEW_OF(&page_music), core, &page_params);
    egui_view_group_init_with_params(EGUI_VIEW_OF(&page_dashboard), core, &page_params);
    egui_view_group_init_with_params(EGUI_VIEW_OF(&page_watch), core, &page_params);
