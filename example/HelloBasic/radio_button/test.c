...
#define RADIO_VERIFY_RETRY_MAX 3U

static uint8_t runtime_fail_reported;
static uint8_t radio_verify_retries[4];
...
static uint8_t schedule_radio_verify_retry(uint8_t verify_index, egui_sim_action_t *p_action)
{
    if (verify_index >= 4U || radio_verify_retries[verify_index] >= RADIO_VERIFY_RETRY_MAX)
    {
        return 0U;
    }

    radio_verify_retries[verify_index]++;
...
void test_init_ui(egui_core_t *core)
{
    ...
    memset(radio_verify_retries, 0, sizeof(radio_verify_retries));
#endif
