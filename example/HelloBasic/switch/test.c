...
bool egui_port_get_recording_action(int action_index, egui_sim_action_t *p_action)
{
    ...
    {
    case 0:
        if (first_call && (switch_xs.is_checked != 0 || switch_s.is_checked != 1 || switch_m.is_checked != 0 || switch_l.is_checked != 1))
        {
            report_runtime_failure("switch initial state mismatch");
        }
    ...
    case 4:
        if (first_call)
        {
            ...
            {
                report_runtime_failure("switch_l did not toggle off");
            }
            if (switch_xs.is_checked != 1 || switch_s.is_checked != 0 || switch_m.is_checked != 1 || switch_l.is_checked != 0)
            {
                report_runtime_failure("switch final state mismatch");
            }
