...
static const char *demo_machine_titles[24] = {"Inbound", "Mixer", "Blend",  "Cook",   "Fill",   "Seal",   "Washer", "Buffer",
                                              "Convey",  "AGV",   "Sorter", "Pallet", "Chill",  "Press",  "Label",  "Vision",
                                              "Stack",   "Pack",  "Dock 1", "Dock 2", "Rework", "QC",     "Spare",  "Ship"};
...
static const uint16_t demo_order_eta_minutes[8] = {8U * 60U + 40U, 9U * 60U + 5U, 9U * 60U + 20U, 10U * 60U, 10U * 60U + 25U, 11U * 60U + 10U,
                                                    11U * 60U + 35U, 12U * 60U + 15U};
...
EGUI_VIEW_VIRTUAL_STAGE_PARAMS_INIT(virtual_stage_params, DEMO_PAGE_X, DEMO_PAGE_Y, DEMO_PAGE_W, DEMO_PAGE_H);

EGUI_BACKGROUND_GRADIENT_PARAM_INIT(screen_bg_param, EGUI_BACKGROUND_GRADIENT_DIR_VERTICAL, EGUI_COLOR_HEX(0xEEF4F7), EGUI_COLOR_HEX(0xD8E7EE),
                                    EGUI_ALPHA_100);
...
egui_view_t *demo_find_live_view(uint32_t stable_id)
{
    ...
    {
        const egui_view_virtual_stage_slot_t *slot = egui_view_virtual_stage_get_slot(EGUI_VIEW_OF(&virtual_stage), i);
        if (slot != NULL && slot->state != EGUI_VIEW_VIRTUAL_STAGE_SLOT_STATE_UNUSED && slot->stable_id == stable_id && slot->view != NULL)
        {
            return slot->view;
        }
    }
...
static void demo_notify_range(uint32_t base_id, uint32_t count)
{
    ...
    for (i = 0; i < count; i++)
    {
        egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), base_id + i);
    }
}
...
static void demo_notify_operational_nodes(void)
{
    ...
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_TABLE_SUMMARY_ID);
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_QUICK_MATRIX_ID);
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_SPANGROUP_NOTE_ID);
}
...
static void demo_notify_text_dependents(uint32_t stable_id)
{
    ...
    {
        demo_notify_range(DEMO_ORDER_BASE_ID, EGUI_ARRAY_SIZE(demo_order_ids));
        egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_KPI_BASE_ID + 6U);
        egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_KPI_BASE_ID + 7U);
        egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_SPANGROUP_NOTE_ID);
        return;
    }
    ...
    {
        egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_ALERT_BASE_ID);
        egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_KPI_BASE_ID + 7U);
        egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_SPANGROUP_NOTE_ID);
    }
}
...
{
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_ACTION_SHIFT_A_ID);
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_ACTION_SHIFT_B_ID);
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_KPI_BASE_ID + 1U);
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_SCHEDULE_BASE_ID + 0U);
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_SCHEDULE_BASE_ID + 1U);
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_ALERT_BASE_ID + 6U);
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_SPANGROUP_NOTE_ID);
}

static void demo_notify_zone_nodes(uint32_t stable_id)
{
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), stable_id);
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_KPI_BASE_ID + 2U);
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_ALERT_BASE_ID + 3U);
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_TABLE_SUMMARY_ID);
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_SPANGROUP_NOTE_ID);
}
...
{
    int value = 24 + demo_context.slider_value[1] / 2 + (demo_pack_is_bag() ? 12 : 0) + (int)EGUI_MIN((uint16_t)(demo_context.export_count * 6U), (uint16_t)24) -
                (demo_context.switch_enabled[1] ? 6 : 0);
...
static const char *demo_get_machine_status(const demo_virtual_node_t *node, char *buffer, size_t buffer_size)
{
    ...
    {
        demo_copy_text(buffer, buffer_size, demo_context.checkbox_enabled[0] ? "QA audit" : (demo_machine_is_active(node->desc.stable_id) ? "Line chk" : "Vision idle"));
        return buffer;
    }
...
static void demo_init_nodes(void)
{
    ...
    for (i = 0; i < EGUI_ARRAY_SIZE(demo_schedule_titles); i++)
    {
        ...
        demo_init_node(&demo_context.nodes[cursor++], kind, DEMO_SCHEDULE_BASE_ID + i, x, y, DEMO_SCHEDULE_W, DEMO_SCHEDULE_H,
                       (int16_t)(40 + i), EGUI_VIEW_VIRTUAL_STAGE_VIEW_TYPE_NONE, 0, (uint8_t)i, (uint8_t)i, title);
    }
    ...
    for (i = 0; i < EGUI_ARRAY_SIZE(demo_sensor_titles); i++)
    {
        ...
        demo_init_node(&demo_context.nodes[cursor++], kind, DEMO_SENSOR_BASE_ID + i, x, y, DEMO_SENSOR_W, DEMO_SENSOR_H,
                       (int16_t)(110 + i), EGUI_VIEW_VIRTUAL_STAGE_VIEW_TYPE_NONE, 0, (uint8_t)(i + 1U), (uint8_t)i, title);
    }
    ...
    for (i = 0; i < EGUI_ARRAY_SIZE(demo_zone_titles); i++)
    {
        ...
        demo_init_node(&demo_context.nodes[cursor++], DEMO_NODE_KIND_ZONE_BUTTON, DEMO_ZONE_BASE_ID + i, x, y, DEMO_ZONE_W, DEMO_ZONE_H,
                       (int16_t)(130 + i), DEMO_VIEW_TYPE_BUTTON, EGUI_VIRTUAL_STAGE_NODE_FLAG_INTERACTIVE, (uint8_t)(i + 2U), (uint8_t)i,
                       demo_zone_titles[i]);
    }
    ...
    demo_init_node(&demo_context.nodes[cursor++], DEMO_NODE_KIND_TEXTINPUT, DEMO_NOTE_INPUT_ID, DEMO_NOTE_INPUT_X, DEMO_NOTE_INPUT_Y, DEMO_INPUT_W, DEMO_INPUT_H,
                   191, DEMO_VIEW_TYPE_TEXTINPUT, EGUI_VIRTUAL_STAGE_NODE_FLAG_INTERACTIVE, 1, 1, "Note");
    ...
    for (i = 0; i < EGUI_ARRAY_SIZE(demo_segment_titles); i++)
    {
        ...
        demo_init_node(&demo_context.nodes[cursor++], DEMO_NODE_KIND_SEGMENTED, DEMO_SEGMENT_BASE_ID + i, x, DEMO_SEGMENT_START_Y, DEMO_SEGMENT_W,
                       DEMO_SEGMENT_H, (int16_t)(294 + i), DEMO_VIEW_TYPE_SEGMENTED, EGUI_VIRTUAL_STAGE_NODE_FLAG_INTERACTIVE, (uint8_t)(i + 1U),
                       (uint8_t)i, demo_segment_titles[i]);
    }
    ...
    for (i = 0; i < EGUI_ARRAY_SIZE(demo_progress_titles); i++)
    {
        ...
        demo_init_node(&demo_context.nodes[cursor++], DEMO_NODE_KIND_PROGRESS, DEMO_PROGRESS_BASE_ID + i, x, DEMO_PROGRESS_START_Y, DEMO_PROGRESS_W,
                       DEMO_PROGRESS_H, (int16_t)(296 + i), EGUI_VIEW_VIRTUAL_STAGE_VIEW_TYPE_NONE, 0, (uint8_t)(i + 2U), (uint8_t)i,
                       demo_progress_titles[i]);
    }
    ...
    for (i = 0; i < EGUI_ARRAY_SIZE(demo_roller_titles); i++)
    {
        ...
        demo_init_node(&demo_context.nodes[cursor++], DEMO_NODE_KIND_ROLLER, DEMO_ROLLER_BASE_ID + i, x, DEMO_ROLLER_START_Y, DEMO_ROLLER_W,
                       DEMO_ROLLER_H, (int16_t)(298 + i), DEMO_VIEW_TYPE_ROLLER, EGUI_VIRTUAL_STAGE_NODE_FLAG_INTERACTIVE, (uint8_t)(i + 3U),
                       (uint8_t)i, demo_roller_titles[i]);
    }
    ...
    for (i = 0; i < EGUI_ARRAY_SIZE(demo_slider_titles); i++)
    {
        ...
        demo_init_node(&demo_context.nodes[cursor++], DEMO_NODE_KIND_SLIDER, DEMO_SLIDER_BASE_ID + i, x, DEMO_SLIDER_START_Y, DEMO_RACK_WIDGET_W,
                       DEMO_RACK_WIDGET_H, (int16_t)(290 + i), DEMO_VIEW_TYPE_SLIDER, EGUI_VIRTUAL_STAGE_NODE_FLAG_INTERACTIVE, (uint8_t)(i + 1U),
                       (uint8_t)i, demo_slider_titles[i]);
    }
    ...
    for (i = 0; i < EGUI_ARRAY_SIZE(demo_toggle_titles); i++)
    {
        ...
        demo_init_node(&demo_context.nodes[cursor++], DEMO_NODE_KIND_TOGGLE, DEMO_TOGGLE_BASE_ID + i, x, DEMO_TOGGLE_START_Y, DEMO_RACK_WIDGET_W,
                       DEMO_RACK_WIDGET_H, (int16_t)(300 + i), DEMO_VIEW_TYPE_TOGGLE, EGUI_VIRTUAL_STAGE_NODE_FLAG_INTERACTIVE, (uint8_t)(i + 2U),
                       (uint8_t)i, demo_toggle_titles[i]);
    }
    ...
    for (i = 0; i < EGUI_ARRAY_SIZE(demo_picker_titles); i++)
    {
        ...
        demo_init_node(&demo_context.nodes[cursor++], DEMO_NODE_KIND_NUMBER_PICKER, DEMO_PICKER_BASE_ID + i, x, DEMO_PICKER_START_Y, DEMO_PICKER_W,
                       DEMO_PICKER_H, (int16_t)(310 + i), DEMO_VIEW_TYPE_NUMBER_PICKER, EGUI_VIRTUAL_STAGE_NODE_FLAG_INTERACTIVE, (uint8_t)(i + 3U),
                       (uint8_t)i, demo_picker_titles[i]);
    }
...
void demo_textinput_focus_changed(egui_view_t *self, int is_focused)
{
    ...
    egui_view_request_layout(EGUI_VIEW_OF(&virtual_stage));
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_TABLE_SUMMARY_ID);
    demo_refresh_status();
}
...
void demo_button_click_cb(egui_view_t *self)
{
    ...
    else if (button_view->stable_id == DEMO_ACTION_EXPORT_ID)
    {
        ...
        demo_notify_operational_nodes();
        egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_ACTION_EXPORT_ID);
    }
...
static void demo_toggle_pin(uint32_t stable_id, uint32_t action_id, uint8_t *enabled_flag)
{
    if (*enabled_flag)
    {
        egui_view_virtual_stage_unpin_node(EGUI_VIEW_OF(&virtual_stage), stable_id);
        *enabled_flag = 0;
    }
    else if (egui_view_virtual_stage_pin_node(EGUI_VIEW_OF(&virtual_stage), stable_id))
    {
        *enabled_flag = 1;
    }
    ...
    demo_notify_operational_nodes();
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), action_id);
    EGUI_VIEW_OF(&virtual_stage)->api->calculate_layout(EGUI_VIEW_OF(&virtual_stage));
}
...
static void demo_reset_business_state(void)
{
    ...
    egui_view_virtual_stage_unpin_node(EGUI_VIEW_OF(&virtual_stage), DEMO_MACHINE_MIXER_ID);
    egui_view_virtual_stage_unpin_node(EGUI_VIEW_OF(&virtual_stage), DEMO_MACHINE_AGV_ID);
#if EGUI_CONFIG_FUNCTION_SUPPORT_FOCUS
    egui_focus_manager_clear_focus();
#endif
    egui_view_virtual_stage_notify_data_changed(EGUI_VIEW_OF(&virtual_stage));
}
...
static void demo_draw_kpi_node(const demo_virtual_node_t *node, const egui_region_t *region)
{
    ...
    egui_canvas_draw_text_in_rect(DEMO_FONT_BUTTON, value_text, &value_region, EGUI_ALIGN_LEFT | EGUI_ALIGN_BOTTOM, DEMO_COLOR_TEXT_PRIMARY,
                                  EGUI_ALPHA_100);
}
...
static void demo_draw_window_node(const demo_virtual_node_t *node, const egui_region_t *region)
{
    ...
    egui_canvas_draw_text_in_rect(DEMO_FONT_CAP, demo_get_line_text(), &body_region, EGUI_ALIGN_LEFT | EGUI_ALIGN_VCENTER, DEMO_COLOR_TEXT_PRIMARY, EGUI_ALPHA_100);
...
static void demo_draw_list_node(const demo_virtual_node_t *node, const egui_region_t *region)
{
    ...
    for (i = 0; i < 3U; i++)
    {
        ...
        egui_canvas_draw_round_rectangle_fill(row_region.location.x, row_region.location.y, row_region.size.width, row_region.size.height, 2, fill, EGUI_ALPHA_100);
...
static void demo_draw_machine_node(const demo_virtual_node_t *node, const egui_region_t *region)
{
    ...
    demo_draw_progress_bar(region->location.x + 6, region->location.y + region->size.height - 7, region->size.width - 12, load, border, EGUI_COLOR_HEX(0xD8E1E8));
...
static void demo_draw_order_node(const demo_virtual_node_t *node, const egui_region_t *region)
{
    ...
    egui_canvas_draw_text_in_rect(DEMO_FONT_CAP, demo_get_order_step_text(node->data_index, step_text, sizeof(step_text)), &step_region,
                                  EGUI_ALIGN_LEFT | EGUI_ALIGN_BOTTOM, DEMO_COLOR_TEXT_MUTED,
                                  EGUI_ALPHA_100);
...
static void demo_draw_toggle_node(const demo_virtual_node_t *node, const egui_region_t *region)
{
    ...
    egui_canvas_draw_text_in_rect(EGUI_FONT_ICON_MS_16, demo_get_toggle_icon(node->desc.stable_id), &icon_region, EGUI_ALIGN_CENTER, EGUI_COLOR_WHITE, EGUI_ALPHA_100);
...
static void demo_draw_progress_node(const demo_virtual_node_t *node, const egui_region_t *region)
{
    ...
    demo_draw_progress_bar(region->location.x + 6, region->location.y + region->size.height - 6, region->size.width - 12, value, fill, EGUI_COLOR_HEX(0xD6E0E8));
}
...
static void demo_draw_badge_group_node(const demo_virtual_node_t *node, const egui_region_t *region)
{
    ...
    snprintf(badge_text[1], sizeof(badge_text[1]), "L%u", (unsigned)egui_view_virtual_stage_get_slot_count(EGUI_VIEW_OF(&virtual_stage)));
...
static void demo_draw_message_bar_node(const demo_virtual_node_t *node, const egui_region_t *region)
{
    ...
    egui_canvas_draw_text_in_rect(DEMO_FONT_CAP, demo_alert_times[demo_context.quick_matrix_index], &time_region, EGUI_ALIGN_RIGHT | EGUI_ALIGN_VCENTER, text_color,
                                  EGUI_ALPHA_100);
}
...
static void demo_draw_digital_clock_node(const demo_virtual_node_t *node, const egui_region_t *region)
{
    ...
    egui_canvas_draw_round_rectangle_fill(region->location.x + 6, region->location.y + 3, 4, region->size.height - 6, 2, EGUI_COLOR_HEX(0x5B98E5), EGUI_ALPHA_100);
...
static void demo_draw_stopwatch_node(const demo_virtual_node_t *node, const egui_region_t *region)
{
    ...
    uint8_t is_running = (uint8_t)(demo_context.shift_a_enabled || demo_context.shift_b_enabled || demo_context.quick_matrix_index != 0U ||
                                   demo_context.export_count != 0U);
    char time_text[16];
    int32_t center_x = region->location.x + 18;
    int32_t center_y = region->location.y + region->size.height / 2;
    int32_t hand_x;
...
static void demo_refresh_status(void)
{
    uint8_t live_slots = egui_view_virtual_stage_get_slot_count(EGUI_VIEW_OF(&virtual_stage));
    uint16_t live_bytes = demo_get_live_view_bytes();
    uint16_t naive_bytes = demo_get_naive_view_bytes();
    ...
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_SCHEDULE_BASE_ID + 0U);
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_TABLE_SUMMARY_ID);
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_SCHEDULE_BASE_ID + 5U);
    egui_view_virtual_stage_notify_node_changed(EGUI_VIEW_OF(&virtual_stage), DEMO_SPANGROUP_NOTE_ID);
}
...
void test_init_ui(void)
{
    ...
    egui_view_virtual_stage_init_with_params(EGUI_VIEW_OF(&virtual_stage), &virtual_stage_params);
    egui_view_virtual_stage_set_live_slot_limit(EGUI_VIEW_OF(&virtual_stage), DEMO_LIVE_SLOT_LIMIT);
    egui_view_virtual_stage_set_adapter(EGUI_VIEW_OF(&virtual_stage), &demo_adapter, &demo_context);
    egui_view_set_background(EGUI_VIEW_OF(&virtual_stage), EGUI_BG_OF(&page_bg));
    egui_view_override_api_on_touch(EGUI_VIEW_OF(&virtual_stage), &virtual_stage_touch_api, demo_page_touch_cb);
    egui_view_set_on_click_listener(EGUI_VIEW_OF(&virtual_stage), demo_page_click_cb);

    demo_refresh_status();

    egui_core_add_user_root_view(EGUI_VIEW_OF(&background_view));
    egui_core_add_user_root_view(EGUI_VIEW_OF(&virtual_stage));
...
{
    if (egui_view_virtual_stage_get_slot_count(EGUI_VIEW_OF(&virtual_stage)) > DEMO_LIVE_SLOT_LIMIT)
    {
...
{
    demo_virtual_node_t *node = demo_find_node(stable_id);
    egui_view_t *page = EGUI_VIEW_OF(&virtual_stage);
    ...
    *x = page->region_screen.location.x + node->desc.region.location.x + node->desc.region.size.width / 2;
    *y = page->region_screen.location.y + node->desc.region.location.y + node->desc.region.size.height / 2;
}
...
static void demo_get_node_pos(uint32_t stable_id, float rel_x, float rel_y, int *x, int *y)
{
    ...
    *x = page->region_screen.location.x + node->desc.region.location.x + (int)(node->desc.region.size.width * rel_x);
    *y = page->region_screen.location.y + node->desc.region.location.y + (int)(node->desc.region.size.height * rel_y);
}
...
bool egui_port_get_recording_action(int action_index, egui_sim_action_t *p_action)
{
    ...
    case 0:
        if (first_call)
        {
            if (egui_view_virtual_stage_get_slot_count(EGUI_VIEW_OF(&virtual_stage)) != 0)
            {
    ...
    case 8:
        if (first_call)
        {
            ...
            else if (egui_view_virtual_stage_get_slot_count(EGUI_VIEW_OF(&virtual_stage)) != 1)
            {
    ...
    case 12:
        if (first_call)
        {
            ...
            if (egui_view_virtual_stage_get_slot_count(EGUI_VIEW_OF(&virtual_stage)) != 1)
            {
    ...
    case 18:
        if (first_call)
        {
            ...
            else if (egui_view_virtual_stage_get_slot_count(EGUI_VIEW_OF(&virtual_stage)) != 2)
            {
    ...
    case 23:
        if (first_call)
        {
            ...
            EGUI_VIEW_OF(&virtual_stage)->api->calculate_layout(EGUI_VIEW_OF(&virtual_stage));
            if (egui_view_virtual_stage_get_slot_count(EGUI_VIEW_OF(&virtual_stage)) < 2)
            {
    ...
    case 28:
        if (first_call)
        {
            ...
            if (egui_view_virtual_stage_get_slot_count(EGUI_VIEW_OF(&virtual_stage)) > 3)
            {
    ...
    case 30:
        if (first_call)
        {
            ...
            if (egui_view_virtual_stage_get_slot_count(EGUI_VIEW_OF(&virtual_stage)) > 2U)
            {
