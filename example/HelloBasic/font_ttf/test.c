...
EGUI_VIEW_LABEL_PARAMS_INIT(s_title_params, 0, 4, SCREEN_W, 28, "Runtime TTF Font", EGUI_CONFIG_FONT_DEFAULT, EGUI_THEME_TEXT_PRIMARY, EGUI_ALPHA_100);
...
void test_init_ui(egui_core_t *core)
{
    ...
    if (font_ok && egui_font_ttf_init(&s_font_sm, s_ttf_buf, s_ttf_size, 16) == 0 && egui_font_ttf_init(&s_font_md, s_ttf_buf, s_ttf_size, 24) == 0 &&
        egui_font_ttf_init(&s_font_lg, s_ttf_buf, s_ttf_size, 32) == 0)
    {
        /* Small label */
        egui_view_label_init(EGUI_VIEW_OF(&s_label_sm), core);
        egui_view_set_position(EGUI_VIEW_OF(&s_label_sm), 4, 38);
        egui_view_set_size(EGUI_VIEW_OF(&s_label_sm), SCREEN_W - 8, 24);
        egui_view_label_set_text(EGUI_VIEW_OF(&s_label_sm), "Hello, TTF 16px");
        egui_view_label_set_font(EGUI_VIEW_OF(&s_label_sm), (egui_font_t *)&s_font_sm);
        egui_view_label_set_font_color(EGUI_VIEW_OF(&s_label_sm), EGUI_COLOR_WHITE, EGUI_ALPHA_100);
        egui_core_add_user_root_view(EGUI_VIEW_OF(&s_label_sm));

        /* Medium label */
        egui_view_label_init(EGUI_VIEW_OF(&s_label_md), core);
        egui_view_set_position(EGUI_VIEW_OF(&s_label_md), 4, 68);
        egui_view_set_size(EGUI_VIEW_OF(&s_label_md), SCREEN_W - 8, 36);
        egui_view_label_set_text(EGUI_VIEW_OF(&s_label_md), "Hello, TTF 24px");
        egui_view_label_set_font(EGUI_VIEW_OF(&s_label_md), (egui_font_t *)&s_font_md);
        egui_view_label_set_font_color(EGUI_VIEW_OF(&s_label_md), EGUI_COLOR_WHITE, EGUI_ALPHA_100);
        egui_core_add_user_root_view(EGUI_VIEW_OF(&s_label_md));

        /* Large label */
        egui_view_label_init(EGUI_VIEW_OF(&s_label_lg), core);
        egui_view_set_position(EGUI_VIEW_OF(&s_label_lg), 4, 110);
        egui_view_set_size(EGUI_VIEW_OF(&s_label_lg), SCREEN_W - 8, 48);
        egui_view_label_set_text(EGUI_VIEW_OF(&s_label_lg), "Hello, 32px");
        egui_view_label_set_font(EGUI_VIEW_OF(&s_label_lg), (egui_font_t *)&s_font_lg);
        egui_view_label_set_font_color(EGUI_VIEW_OF(&s_label_lg), EGUI_COLOR_WHITE, EGUI_ALPHA_100);
        egui_core_add_user_root_view(EGUI_VIEW_OF(&s_label_lg));

        /* Status */
        egui_view_label_init(EGUI_VIEW_OF(&s_status_label), core);
        egui_view_set_position(EGUI_VIEW_OF(&s_status_label), 4, SCREEN_H - 24);
        egui_view_set_size(EGUI_VIEW_OF(&s_status_label), SCREEN_W - 8, 20);
        egui_view_label_set_text(EGUI_VIEW_OF(&s_status_label), "DejaVuSans.ttf loaded OK");
        egui_view_label_set_align_type(EGUI_VIEW_OF(&s_status_label), EGUI_ALIGN_CENTER);
    ...
    else
    {
        ...
        egui_view_label_init(EGUI_VIEW_OF(&s_status_label), core);
        egui_view_set_position(EGUI_VIEW_OF(&s_status_label), 4, SCREEN_H / 2 - 12);
        egui_view_set_size(EGUI_VIEW_OF(&s_status_label), SCREEN_W - 8, 24);
        egui_view_label_set_text(EGUI_VIEW_OF(&s_status_label), "TTF font not found");
        egui_view_label_set_align_type(EGUI_VIEW_OF(&s_status_label), EGUI_ALIGN_CENTER);
        egui_core_add_user_root_view(EGUI_VIEW_OF(&s_status_label));
    }
}
