...
`virtual_tree_basic` 只保留一个 header、一个操作条和 `root / group / task` 三层最小树结构，重点演示最小接入闭环：`init_with_setup`、点击回查 node、branch 折叠/展开、task 单项 patch、结构刷新和 jump / reset。
...
```bash
make -j1 all APP=HelloVirtual APP_SUB=virtual_tree PORT=pc
python scripts/code_runtime_check.py --app HelloVirtual --app-sub virtual_tree --keep-screenshots
```

截图输出目录：

```bash
runtime_check_output/HelloVirtual_virtual_tree/default/
```
...
- `example/HelloVirtual/virtual_tree/test.c`
