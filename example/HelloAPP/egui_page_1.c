...
static void egui_page_1_timer_callback(egui_timer_t *timer)
{
    egui_page_1_t *local = (egui_page_1_t *)timer->user_data;
    EGUI_UNUSED(local);
    EGUI_LOG_INF("egui_page_1_timer_callback\n");

    local->index += 1;
    egui_api_sprintf(local->label_str, "Time: %d", local->index);
...
void egui_page_1_on_open(egui_page_base_t *self)
{
    ...
    egui_view_set_size((egui_view_t *)&local->label_1, EGUI_CONFIG_SCREEN_WIDTH, 0);
    egui_view_set_margin_all((egui_view_t *)&local->label_1, 5);
    egui_view_label_set_text((egui_view_t *)&local->label_1, "egui_page_1");
    egui_view_label_set_align_type((egui_view_t *)&local->label_1, EGUI_ALIGN_CENTER);
    egui_view_label_set_font_with_std_height((egui_view_t *)&local->label_1, (egui_font_t *)&egui_res_font_dejavusans_30_4);
    ...
    // image_1
    egui_utils_view_image_std_normal_init((egui_view_t *)&local->image_1, core, (egui_image_t *)&egui_res_image_test_rgb565_4, 0, 0);
    ...
    egui_view_group_add_child((egui_view_t *)&local->layout_1, (egui_view_t *)&local->image_1);
...
void egui_page_1_on_close(egui_page_base_t *self)
{
    egui_page_1_t *local = (egui_page_1_t *)self;
    EGUI_UNUSED(local);
...
static const egui_page_base_api_t EGUI_VIEW_API_TABLE_NAME(egui_page_1_t) = {
        .on_open = egui_page_1_on_open,               // changed
        .on_close = egui_page_1_on_close,             // changed
...
void egui_page_1_init(egui_page_base_t *self, egui_core_t *core)
{
    ...
    // init local data.
    local->index = 0;
    egui_timer_init_timer(&local->timer, (void *)local, egui_page_1_timer_callback);

    egui_page_base_set_name(self, "egui_page_1");
}
