...
static egui_view_skeleton_loader_t loader_primary;
static egui_view_label_t status_label;
static egui_view_line_t section_divider;
static egui_view_linearlayout_t bottom_row;
static egui_view_linearlayout_t compact_column;
static egui_view_skeleton_loader_t loader_compact;
static egui_view_label_t compact_label;
static egui_view_linearlayout_t locked_column;
static egui_view_skeleton_loader_t loader_locked;
static egui_view_label_t locked_label;

static const char *title_text = "Skeleton Loader";
...
static void apply_compact_state(uint8_t index, uint8_t is_active)
{
    ...
    egui_view_skeleton_loader_set_focus_block(EGUI_VIEW_OF(&loader_compact), (index % 2) == 0 ? 3 : 0);
    if (is_active)
    {
...
{
    uint8_t next = (egui_view_skeleton_loader_get_current_snapshot(self) + 1) % 2;
    egui_view_skeleton_loader_set_current_snapshot(self, next);
    egui_view_skeleton_loader_set_focus_block(self, next == 0 ? 5 : 0);
    apply_compact_state(egui_view_skeleton_loader_get_current_snapshot(EGUI_VIEW_OF(&loader_compact)), 0);
    set_status(next == 0 ? "Layout A phase" : "Layout B phase", EGUI_COLOR_HEX(0x38BDF8));
}
...
static void on_compact_click(egui_view_t *self)
{
    ...
    set_status(next == 0 ? "Compact A load" : "Compact B load", EGUI_COLOR_HEX(0xF59E0B));
}
...
void test_init_ui(void)
{
    ...
    egui_view_skeleton_loader_init(EGUI_VIEW_OF(&loader_primary));
    egui_view_set_size(EGUI_VIEW_OF(&loader_primary), 176, 132);
    ...
    egui_view_label_set_text(EGUI_VIEW_OF(&status_label), "Layout A phase");
    ...
    egui_view_skeleton_loader_init(EGUI_VIEW_OF(&loader_compact));
    egui_view_set_size(EGUI_VIEW_OF(&loader_compact), 106, 92);
    egui_view_skeleton_loader_set_snapshots(EGUI_VIEW_OF(&loader_compact), compact_snapshots, 2);
    egui_view_skeleton_loader_set_current_snapshot(EGUI_VIEW_OF(&loader_compact), 0);
    egui_view_skeleton_loader_set_font(EGUI_VIEW_OF(&loader_compact), (const egui_font_t *)&egui_res_font_montserrat_8_4);
    egui_view_skeleton_loader_set_show_header(EGUI_VIEW_OF(&loader_compact), 0);
    egui_view_skeleton_loader_set_compact_mode(EGUI_VIEW_OF(&loader_compact), 1);
    apply_compact_state(0, 0);
    egui_view_set_on_click_listener(EGUI_VIEW_OF(&loader_compact), on_compact_click);
    egui_view_group_add_child(EGUI_VIEW_OF(&compact_column), EGUI_VIEW_OF(&loader_compact));
    ...
    egui_view_skeleton_loader_init(EGUI_VIEW_OF(&loader_locked));
    egui_view_set_size(EGUI_VIEW_OF(&loader_locked), 106, 92);
    egui_view_skeleton_loader_set_snapshots(EGUI_VIEW_OF(&loader_locked), compact_snapshots, 2);
    egui_view_skeleton_loader_set_current_snapshot(EGUI_VIEW_OF(&loader_locked), 0);
...
bool egui_port_get_recording_action(int action_index, egui_sim_action_t *p_action)
{
    ...
    case 0:
        ...
        return true;
    case 1:
        EGUI_SIM_SET_CLICK_VIEW(p_action, EGUI_VIEW_OF(&loader_primary), 700);
        return true;
    case 2:
        EGUI_SIM_SET_WAIT(p_action, 300);
        return true;
    case 3:
        EGUI_SIM_SET_CLICK_VIEW(p_action, EGUI_VIEW_OF(&loader_compact), 700);
        return true;
    case 4:
        EGUI_SIM_SET_WAIT(p_action, 300);
        return true;
    case 5:
        EGUI_SIM_SET_CLICK_VIEW(p_action, EGUI_VIEW_OF(&loader_locked), 700);
        return true;
    case 6:
