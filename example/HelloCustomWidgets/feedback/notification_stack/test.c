...
static egui_view_notification_stack_t stack_primary;
static egui_view_label_t status_label;
static egui_view_line_t section_divider;
static egui_view_linearlayout_t bottom_row;
static egui_view_linearlayout_t compact_column;
static egui_view_notification_stack_t stack_compact;
static egui_view_label_t compact_label;
static egui_view_linearlayout_t locked_column;
static egui_view_notification_stack_t stack_locked;
static egui_view_label_t locked_label;

static const char *title_text = "Notification Stack";
static const char *guide_text = NOTIFY_GUIDE_TEXT;
static const egui_view_line_point_t section_divider_points[] = {{0, 0}, {NOTIFY_DIVIDER_POINTS, 0}};

static const char *primary_titles_a[] = {"Build delay", "Cache drift", "Sync lag"};
static const char *primary_tags_a[] = {"MED", "LOW", "HIGH"};
static const uint8_t primary_levels_a[] = {1, 0, 2};
static const char *primary_titles_b[] = {"Deploy gate", "Alert storm", "Queue full"};
static const char *primary_tags_b[] = {"LOW", "HIGH", "MED"};
static const uint8_t primary_levels_b[] = {0, 2, 1};
static const egui_view_notification_stack_snapshot_t primary_snapshots[] = {
        {"Ops A", primary_titles_a, primary_tags_a, primary_levels_a, 3, 7, 0},
        {"Ops B", primary_titles_b, primary_tags_b, primary_levels_b, 3, 6, 1},
};

static const char *compact_titles_a[] = {"Ping", "Auth", "Mail"};
static const char *compact_tags_a[] = {"L", "M", "H"};
static const uint8_t compact_levels_a[] = {0, 1, 2};
static const char *compact_titles_b[] = {"Web", "DB", "Ops"};
static const char *compact_tags_b[] = {"M", "H", "L"};
static const uint8_t compact_levels_b[] = {1, 2, 0};
static const egui_view_notification_stack_snapshot_t compact_snapshots[] = {
        {"Compact A", compact_titles_a, compact_tags_a, compact_levels_a, 3, 5, 1},
        {"Compact B", compact_titles_b, compact_tags_b, compact_levels_b, 3, 4, 2},
};
...
{
    egui_view_label_set_text(EGUI_VIEW_OF(&compact_label), NOTIFY_COMPACT_LABEL_TEXT);
    egui_view_notification_stack_set_focus_card(EGUI_VIEW_OF(&stack_compact), (index % 2) == 0 ? 1 : 2);
    if (is_active)
    {
...
{
    uint8_t next = (egui_view_notification_stack_get_current_snapshot(self) + 1) % 2;
...
void test_init_ui(void)
{
    ...
    egui_view_set_size(EGUI_VIEW_OF(&root_layout), NOTIFY_ROOT_WIDTH, NOTIFY_ROOT_HEIGHT);
    egui_view_linearlayout_set_orientation(EGUI_VIEW_OF(&root_layout), 0);
    egui_view_linearlayout_set_align_type(EGUI_VIEW_OF(&root_layout), EGUI_ALIGN_HCENTER);

    egui_view_label_init(EGUI_VIEW_OF(&title_label));
    egui_view_set_size(EGUI_VIEW_OF(&title_label), NOTIFY_ROOT_WIDTH, 18);
    egui_view_label_set_text(EGUI_VIEW_OF(&title_label), title_text);
    egui_view_label_set_align_type(EGUI_VIEW_OF(&title_label), EGUI_ALIGN_CENTER);
    egui_view_label_set_font(EGUI_VIEW_OF(&title_label), (const egui_font_t *)&egui_res_font_montserrat_12_4);
    egui_view_label_set_font_color(EGUI_VIEW_OF(&title_label), EGUI_COLOR_HEX(NOTIFY_TITLE_COLOR), EGUI_ALPHA_100);
    egui_view_group_add_child(EGUI_VIEW_OF(&root_layout), EGUI_VIEW_OF(&title_label));

    egui_view_label_init(EGUI_VIEW_OF(&guide_label));
    egui_view_set_size(EGUI_VIEW_OF(&guide_label), NOTIFY_ROOT_WIDTH, 12);
    egui_view_label_set_text(EGUI_VIEW_OF(&guide_label), guide_text);
    egui_view_label_set_align_type(EGUI_VIEW_OF(&guide_label), EGUI_ALIGN_CENTER);
    egui_view_label_set_font(EGUI_VIEW_OF(&guide_label), (const egui_font_t *)&egui_res_font_montserrat_10_4);
    egui_view_label_set_font_color(EGUI_VIEW_OF(&guide_label), EGUI_COLOR_HEX(NOTIFY_GUIDE_COLOR), EGUI_ALPHA_100);
    egui_view_set_margin(EGUI_VIEW_OF(&guide_label), 0, 0, 0, NOTIFY_GUIDE_MARGIN_BOTTOM);
    egui_view_group_add_child(EGUI_VIEW_OF(&root_layout), EGUI_VIEW_OF(&guide_label));

    egui_view_notification_stack_init(EGUI_VIEW_OF(&stack_primary));
    egui_view_set_size(EGUI_VIEW_OF(&stack_primary), NOTIFY_PRIMARY_WIDTH, NOTIFY_PRIMARY_HEIGHT);
    ...
    egui_view_set_size(EGUI_VIEW_OF(&status_label), NOTIFY_ROOT_WIDTH, 13);
    egui_view_label_set_text(EGUI_VIEW_OF(&status_label), NOTIFY_STATUS_TEXT_A);
    egui_view_label_set_align_type(EGUI_VIEW_OF(&status_label), EGUI_ALIGN_CENTER);
    egui_view_label_set_font(EGUI_VIEW_OF(&status_label), (const egui_font_t *)&egui_res_font_montserrat_10_4);
    egui_view_label_set_font_color(EGUI_VIEW_OF(&status_label), EGUI_COLOR_HEX(NOTIFY_STATUS_PRIMARY_COLOR), EGUI_ALPHA_100);
    egui_view_set_margin(EGUI_VIEW_OF(&status_label), 0, 0, 0, NOTIFY_STATUS_MARGIN_BOTTOM);
    egui_view_group_add_child(EGUI_VIEW_OF(&root_layout), EGUI_VIEW_OF(&status_label));

    egui_view_line_init(EGUI_VIEW_OF(&section_divider));
    egui_view_set_size(EGUI_VIEW_OF(&section_divider), NOTIFY_DIVIDER_WIDTH, 2);
    egui_view_line_set_points(EGUI_VIEW_OF(&section_divider), section_divider_points, 2);
    egui_view_line_set_line_width(EGUI_VIEW_OF(&section_divider), 1);
    egui_view_line_set_line_color(EGUI_VIEW_OF(&section_divider), EGUI_COLOR_HEX(NOTIFY_DIVIDER_PRIMARY));
    egui_view_set_margin(EGUI_VIEW_OF(&section_divider), 0, 0, 0, NOTIFY_DIVIDER_MARGIN_BOTTOM);
    egui_view_group_add_child(EGUI_VIEW_OF(&root_layout), EGUI_VIEW_OF(&section_divider));

    egui_view_linearlayout_init(EGUI_VIEW_OF(&bottom_row));
    egui_view_set_size(EGUI_VIEW_OF(&bottom_row), NOTIFY_BOTTOM_ROW_WIDTH, NOTIFY_BOTTOM_ROW_HEIGHT);
    egui_view_linearlayout_set_orientation(EGUI_VIEW_OF(&bottom_row), 1);
    egui_view_linearlayout_set_align_type(EGUI_VIEW_OF(&bottom_row), EGUI_ALIGN_VCENTER);
    egui_view_group_add_child(EGUI_VIEW_OF(&root_layout), EGUI_VIEW_OF(&bottom_row));

    egui_view_linearlayout_init(EGUI_VIEW_OF(&compact_column));
    egui_view_set_size(EGUI_VIEW_OF(&compact_column), NOTIFY_COLUMN_WIDTH, NOTIFY_COLUMN_HEIGHT);
    egui_view_set_margin(EGUI_VIEW_OF(&compact_column), 0, NOTIFY_COLUMN_GAP, 0, 0);
    egui_view_linearlayout_set_orientation(EGUI_VIEW_OF(&compact_column), 0);
    egui_view_linearlayout_set_align_type(EGUI_VIEW_OF(&compact_column), EGUI_ALIGN_HCENTER);
    egui_view_group_add_child(EGUI_VIEW_OF(&bottom_row), EGUI_VIEW_OF(&compact_column));

    egui_view_label_init(EGUI_VIEW_OF(&compact_label));
    egui_view_set_size(EGUI_VIEW_OF(&compact_label), NOTIFY_COLUMN_WIDTH, 13);
    egui_view_label_set_text(EGUI_VIEW_OF(&compact_label), NOTIFY_COMPACT_LABEL_TEXT);
    ...
    egui_view_notification_stack_init(EGUI_VIEW_OF(&stack_compact));
    egui_view_set_size(EGUI_VIEW_OF(&stack_compact), NOTIFY_COLUMN_WIDTH, NOTIFY_CARD_HEIGHT);
    egui_view_notification_stack_set_snapshots(EGUI_VIEW_OF(&stack_compact), compact_snapshots, 2);
    egui_view_notification_stack_set_current_snapshot(EGUI_VIEW_OF(&stack_compact), 0);
    egui_view_notification_stack_set_font(EGUI_VIEW_OF(&stack_compact), (const egui_font_t *)&egui_res_font_montserrat_8_4);
    egui_view_notification_stack_set_show_header(EGUI_VIEW_OF(&stack_compact), 0);
    egui_view_notification_stack_set_compact_mode(EGUI_VIEW_OF(&stack_compact), 1);
    ...
    egui_view_set_size(EGUI_VIEW_OF(&locked_column), NOTIFY_COLUMN_WIDTH, NOTIFY_COLUMN_HEIGHT);
    egui_view_set_margin(EGUI_VIEW_OF(&locked_column), NOTIFY_COLUMN_GAP, 0, 0, 0);
    ...
    egui_view_set_size(EGUI_VIEW_OF(&locked_label), NOTIFY_COLUMN_WIDTH, 13);
    egui_view_label_set_text(EGUI_VIEW_OF(&locked_label), NOTIFY_LOCKED_LABEL_TEXT);
    ...
    egui_view_label_set_font_color(EGUI_VIEW_OF(&locked_label), EGUI_COLOR_HEX(NOTIFY_LOCKED_LABEL_COLOR), EGUI_ALPHA_100);
    ...
    egui_view_notification_stack_init(EGUI_VIEW_OF(&stack_locked));
    egui_view_set_size(EGUI_VIEW_OF(&stack_locked), NOTIFY_COLUMN_WIDTH, NOTIFY_CARD_HEIGHT);
    egui_view_notification_stack_set_snapshots(EGUI_VIEW_OF(&stack_locked), compact_snapshots, 2);
    egui_view_notification_stack_set_current_snapshot(EGUI_VIEW_OF(&stack_locked), 0);
