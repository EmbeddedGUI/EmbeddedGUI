...
static egui_view_alert_banner_t banner_primary;
static egui_view_label_t status_label;
static egui_view_line_t section_divider;
static egui_view_linearlayout_t bottom_row;
static egui_view_linearlayout_t compact_column;
static egui_view_alert_banner_t banner_compact;
static egui_view_label_t compact_label;
static egui_view_linearlayout_t locked_column;
static egui_view_alert_banner_t banner_locked;
static egui_view_label_t locked_label;

static const char *title_text = "Alert Banner";
static const char *guide_text = "Tap banners to rotate queue";
static const egui_view_line_point_t section_divider_points[] = {{0, 0}, {147, 0}};

static const egui_view_alert_banner_item_t primary_items_a[] = {
        {"Cache drift", "OBS", 0, 1},
        {"Auth retry", "WRN", 1, 0},
        {"DB lag peak", "HOT", 2, 0},
        {"CDN warmup", "INF", 0, 1},
};
static const egui_view_alert_banner_item_t primary_items_b[] = {
        {"Index backlog", "WRN", 1, 0},
        {"Queue drain", "HOT", 2, 0},
        {"Edge sync ok", "OBS", 0, 1},
        {"Billing recover", "INF", 0, 1},
};
static const egui_view_alert_banner_snapshot_t primary_snapshots[] = {
        {"Queue A", primary_items_a, 4, 2},
        {"Queue B", primary_items_b, 4, 1},
};

static const egui_view_alert_banner_item_t compact_items_a[] = {
        {"Lag", NULL, 2, 0},
        {"Burst", NULL, 1, 0},
        {"Ack", NULL, 0, 1},
};
static const egui_view_alert_banner_item_t compact_items_b[] = {
        {"Drain", NULL, 1, 0},
        {"Peak", NULL, 2, 0},
        {"Clear", NULL, 0, 1},
};
static const egui_view_alert_banner_snapshot_t compact_snapshots[] = {
        {"Compact A", compact_items_a, 3, 0},
        {"Compact B", compact_items_b, 3, 1},
};
...
static void apply_compact_state(uint8_t index, uint8_t is_active)
{
    ...
    egui_view_alert_banner_set_focus_item(EGUI_VIEW_OF(&banner_compact), (index % 2) == 0 ? 0 : 1);
    if (is_active)
    {
        egui_view_label_set_font_color(EGUI_VIEW_OF(&compact_label), EGUI_COLOR_HEX(0xF59E0B), EGUI_ALPHA_100);
        egui_view_alert_banner_set_palette(EGUI_VIEW_OF(&banner_compact), EGUI_COLOR_HEX(0x23160A), EGUI_COLOR_HEX(0x8B5E1A), EGUI_COLOR_HEX(0xFDE68A),
    ...
    else
    {
        ...
        egui_view_alert_banner_set_palette(EGUI_VIEW_OF(&banner_compact), EGUI_COLOR_HEX(0x121A28), EGUI_COLOR_HEX(0x3D5068), EGUI_COLOR_HEX(0xCBD5E1),
...
{
    uint8_t next = (egui_view_alert_banner_get_current_snapshot(self) + 1) % 2;
    egui_view_alert_banner_set_current_snapshot(self, next);
    egui_view_alert_banner_set_focus_item(self, next == 0 ? 2 : 1);
    apply_compact_state(egui_view_alert_banner_get_current_snapshot(EGUI_VIEW_OF(&banner_compact)), 0);
    set_status(next == 0 ? "Queue A focus" : "Queue B focus", EGUI_COLOR_HEX(0x38BDF8));
}
...
static void on_compact_click(egui_view_t *self)
{
    ...
    set_status(next == 0 ? "Compact A alerts" : "Compact B alerts", EGUI_COLOR_HEX(0xF59E0B));
}
...
void test_init_ui(void)
{
    ...
    egui_view_alert_banner_init(EGUI_VIEW_OF(&banner_primary));
    egui_view_set_size(EGUI_VIEW_OF(&banner_primary), 176, 132);
    egui_view_alert_banner_set_snapshots(EGUI_VIEW_OF(&banner_primary), primary_snapshots, 2);
    egui_view_alert_banner_set_current_snapshot(EGUI_VIEW_OF(&banner_primary), 0);
    egui_view_alert_banner_set_focus_item(EGUI_VIEW_OF(&banner_primary), 2);
    egui_view_alert_banner_set_palette(EGUI_VIEW_OF(&banner_primary), EGUI_COLOR_HEX(0x0F1728), EGUI_COLOR_HEX(0x536379), EGUI_COLOR_HEX(0xE2E8F0),
                                       EGUI_COLOR_HEX(0x93A5BC), EGUI_COLOR_HEX(0x38BDF8));
    egui_view_set_on_click_listener(EGUI_VIEW_OF(&banner_primary), on_primary_click);
    egui_view_set_margin(EGUI_VIEW_OF(&banner_primary), 0, 0, 0, 4);
    egui_view_group_add_child(EGUI_VIEW_OF(&root_layout), EGUI_VIEW_OF(&banner_primary));

    egui_view_label_init(EGUI_VIEW_OF(&status_label));
    egui_view_set_size(EGUI_VIEW_OF(&status_label), 220, 14);
    egui_view_label_set_text(EGUI_VIEW_OF(&status_label), "Queue A focus");
    ...
    egui_view_alert_banner_init(EGUI_VIEW_OF(&banner_compact));
    egui_view_set_size(EGUI_VIEW_OF(&banner_compact), 106, 92);
    egui_view_alert_banner_set_snapshots(EGUI_VIEW_OF(&banner_compact), compact_snapshots, 2);
    egui_view_alert_banner_set_current_snapshot(EGUI_VIEW_OF(&banner_compact), 0);
    egui_view_alert_banner_set_font(EGUI_VIEW_OF(&banner_compact), (const egui_font_t *)&egui_res_font_montserrat_8_4);
    egui_view_alert_banner_set_show_header(EGUI_VIEW_OF(&banner_compact), 0);
    egui_view_alert_banner_set_compact_mode(EGUI_VIEW_OF(&banner_compact), 1);
    apply_compact_state(0, 0);
    egui_view_set_on_click_listener(EGUI_VIEW_OF(&banner_compact), on_compact_click);
    egui_view_group_add_child(EGUI_VIEW_OF(&compact_column), EGUI_VIEW_OF(&banner_compact));
    ...
    egui_view_alert_banner_init(EGUI_VIEW_OF(&banner_locked));
    egui_view_set_size(EGUI_VIEW_OF(&banner_locked), 106, 92);
    egui_view_alert_banner_set_snapshots(EGUI_VIEW_OF(&banner_locked), compact_snapshots, 2);
    egui_view_alert_banner_set_current_snapshot(EGUI_VIEW_OF(&banner_locked), 0);
    egui_view_alert_banner_set_focus_item(EGUI_VIEW_OF(&banner_locked), 0);
    egui_view_alert_banner_set_show_header(EGUI_VIEW_OF(&banner_locked), 0);
    egui_view_alert_banner_set_compact_mode(EGUI_VIEW_OF(&banner_locked), 1);
    egui_view_alert_banner_set_palette(EGUI_VIEW_OF(&banner_locked), EGUI_COLOR_HEX(0x0E1522), EGUI_COLOR_HEX(0x46566B), EGUI_COLOR_HEX(0xCBD5E1),
                                       EGUI_COLOR_HEX(0x8695A9), EGUI_COLOR_HEX(0x758E83));
    egui_view_set_enable(EGUI_VIEW_OF(&banner_locked), 0);
    egui_view_group_add_child(EGUI_VIEW_OF(&locked_column), EGUI_VIEW_OF(&banner_locked));
...
bool egui_port_get_recording_action(int action_index, egui_sim_action_t *p_action)
{
    ...
    case 0:
        ...
        return true;
    case 1:
        EGUI_SIM_SET_CLICK_VIEW(p_action, EGUI_VIEW_OF(&banner_primary), 700);
        return true;
    case 2:
        EGUI_SIM_SET_WAIT(p_action, 300);
        return true;
    case 3:
        EGUI_SIM_SET_CLICK_VIEW(p_action, EGUI_VIEW_OF(&banner_compact), 700);
        return true;
    case 4:
        EGUI_SIM_SET_WAIT(p_action, 300);
        return true;
    case 5:
        EGUI_SIM_SET_CLICK_VIEW(p_action, EGUI_VIEW_OF(&banner_locked), 700);
        return true;
    case 6:
