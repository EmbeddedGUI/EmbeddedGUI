...
void egui_view_server_rack_set_palette(
        egui_view_t *self,
...
static void egui_view_server_rack_on_draw(egui_view_t *self)
{
    ...
    for (i = 0; i < unit_count; i++)
    {
        ...
        egui_canvas_draw_round_rectangle(unit_x, unit_y, unit_w, unit_h, 4, is_current ? 2 : 1, border_color, egui_color_alpha_mix(self->alpha, is_current ? EGUI_ALPHA_80 : EGUI_ALPHA_40));
        ...
        egui_canvas_draw_round_rectangle_fill(meter_x, meter_y, meter_w, meter_h, 3, egui_rgb_mix(local->surface_color, local->border_color, EGUI_ALPHA_20), egui_color_alpha_mix(self->alpha, EGUI_ALPHA_30));
        egui_canvas_draw_round_rectangle_fill(meter_x, meter_y, (meter_w * unit->meter_level) / 100, meter_h, 3, status_color, egui_color_alpha_mix(self->alpha, EGUI_ALPHA_80));
        ...
        if (!local->compact_mode && unit->label != NULL)
        {
            ...
            egui_canvas_draw_text_in_rect(local->font, unit->label, &text_region, EGUI_ALIGN_LEFT, is_focus ? local->text_color : local->muted_text_color, self->alpha);
        }
    }
