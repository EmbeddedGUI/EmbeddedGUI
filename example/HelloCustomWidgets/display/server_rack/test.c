...
static egui_view_server_rack_t rack_primary;
static egui_view_label_t status_label;
static egui_view_line_t section_divider;
static egui_view_linearlayout_t bottom_row;
static egui_view_linearlayout_t compact_column;
static egui_view_server_rack_t rack_compact;
static egui_view_label_t compact_label;
static egui_view_linearlayout_t locked_column;
static egui_view_server_rack_t rack_locked;
static egui_view_label_t locked_label;

static const char *title_text = "Server Rack";
static const char *guide_text = "Tap racks to rotate load";
...
static const egui_view_server_rack_snapshot_t primary_snapshots[] = {
        ...
        {"Rack B", primary_units_b, 5, 1},
};

static const egui_view_server_rack_unit_t compact_units_a[] = {
        {2, 14, 28, 0, "A"},
        {20, 14, 54, 1, "B"},
        {38, 14, 72, 2, "C"},
        {56, 14, 40, 0, "D"},
};
static const egui_view_server_rack_unit_t compact_units_b[] = {
        {2, 14, 36, 1, "A"},
        {20, 14, 64, 2, "B"},
        {38, 14, 46, 0, "C"},
        {56, 14, 78, 1, "D"},
};
static const egui_view_server_rack_snapshot_t compact_snapshots[] = {
        {"Compact A", compact_units_a, 4, 2},
        {"Compact B", compact_units_b, 4, 1},
};
...
static void apply_compact_state(uint8_t index, uint8_t is_active)
{
    ...
    egui_view_server_rack_set_focus_unit(EGUI_VIEW_OF(&rack_compact), (index % 2) == 0 ? 2 : 1);
    if (is_active)
    {
...
{
    uint8_t next = (egui_view_server_rack_get_current_snapshot(self) + 1) % 2;
    egui_view_server_rack_set_current_snapshot(self, next);
    egui_view_server_rack_set_focus_unit(self, next == 0 ? 2 : 1);
    apply_compact_state(egui_view_server_rack_get_current_snapshot(EGUI_VIEW_OF(&rack_compact)), 0);
    set_status(next == 0 ? "Rack A load" : "Rack B load", EGUI_COLOR_HEX(0x38BDF8));
}
...
static void on_compact_click(egui_view_t *self)
{
    ...
    set_status(next == 0 ? "Compact A rack" : "Compact B rack", EGUI_COLOR_HEX(0xF59E0B));
}
...
void test_init_ui(void)
{
    ...
    egui_view_server_rack_init(EGUI_VIEW_OF(&rack_primary));
    egui_view_set_size(EGUI_VIEW_OF(&rack_primary), 176, 132);
    ...
    egui_view_label_set_text(EGUI_VIEW_OF(&status_label), "Rack A load");
    ...
    egui_view_server_rack_init(EGUI_VIEW_OF(&rack_compact));
    egui_view_set_size(EGUI_VIEW_OF(&rack_compact), 106, 92);
    egui_view_server_rack_set_snapshots(EGUI_VIEW_OF(&rack_compact), compact_snapshots, 2);
    egui_view_server_rack_set_current_snapshot(EGUI_VIEW_OF(&rack_compact), 0);
    egui_view_server_rack_set_font(EGUI_VIEW_OF(&rack_compact), (const egui_font_t *)&egui_res_font_montserrat_8_4);
    egui_view_server_rack_set_show_header(EGUI_VIEW_OF(&rack_compact), 0);
    egui_view_server_rack_set_compact_mode(EGUI_VIEW_OF(&rack_compact), 1);
    apply_compact_state(0, 0);
    egui_view_set_on_click_listener(EGUI_VIEW_OF(&rack_compact), on_compact_click);
    egui_view_group_add_child(EGUI_VIEW_OF(&compact_column), EGUI_VIEW_OF(&rack_compact));
    ...
    egui_view_server_rack_init(EGUI_VIEW_OF(&rack_locked));
    egui_view_set_size(EGUI_VIEW_OF(&rack_locked), 106, 92);
    egui_view_server_rack_set_snapshots(EGUI_VIEW_OF(&rack_locked), compact_snapshots, 2);
    egui_view_server_rack_set_current_snapshot(EGUI_VIEW_OF(&rack_locked), 0);
...
bool egui_port_get_recording_action(int action_index, egui_sim_action_t *p_action)
{
    ...
    case 0:
        ...
        return true;
    case 1:
        EGUI_SIM_SET_CLICK_VIEW(p_action, EGUI_VIEW_OF(&rack_primary), 700);
        return true;
    case 2:
        EGUI_SIM_SET_WAIT(p_action, 300);
        return true;
    case 3:
        EGUI_SIM_SET_CLICK_VIEW(p_action, EGUI_VIEW_OF(&rack_compact), 700);
        return true;
    case 4:
        EGUI_SIM_SET_WAIT(p_action, 300);
        return true;
    case 5:
        EGUI_SIM_SET_CLICK_VIEW(p_action, EGUI_VIEW_OF(&rack_locked), 700);
        return true;
    case 6:
