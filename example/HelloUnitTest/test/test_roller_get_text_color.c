...
#include "test_roller_get_text_color.h"
...
{
    egui_color_t got = egui_view_roller_get_text_color(NULL);
    EGUI_TEST_ASSERT_EQUAL_INT(0, (int)got.full);
}

void test_roller_get_text_color_run(void)
{
    EGUI_TEST_SUITE_BEGIN(roller_get_text_color);

    EGUI_TEST_RUN(test_roller_get_text_color_default);
    EGUI_TEST_RUN(test_roller_get_text_color_after_set);
    EGUI_TEST_RUN(test_roller_get_text_color_update);
    EGUI_TEST_RUN(test_roller_get_text_color_null_self);
