...
#include "test_tab_bar_get_indicator_color.h"
...
/* Default indicator_color after init is EGUI_THEME_PRIMARY. */
...
static void test_tab_bar_get_indicator_color_after_set(void)
{
    egui_color_t c;
    c.full = 0x556677u;
    setup();
...
static void test_tab_bar_get_indicator_color_update(void)
{
    egui_color_t c1, c2;
    c1.full = 0x556677u;
    c2.full = 0x889900u;
...
static void test_tab_bar_get_indicator_color_null_self(void)
{
    egui_color_t got = egui_view_tab_bar_get_indicator_color(NULL);
    EGUI_TEST_ASSERT_EQUAL_INT(0, (int)got.full);
}

void test_tab_bar_get_indicator_color_run(void)
{
    EGUI_TEST_SUITE_BEGIN(tab_bar_get_indicator_color);

    EGUI_TEST_RUN(test_tab_bar_get_indicator_color_default);
    EGUI_TEST_RUN(test_tab_bar_get_indicator_color_after_set);
    EGUI_TEST_RUN(test_tab_bar_get_indicator_color_update);
    EGUI_TEST_RUN(test_tab_bar_get_indicator_color_null_self);
