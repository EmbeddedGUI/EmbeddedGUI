...
{
    setup();
    EGUI_TEST_ASSERT_EQUAL_INT((int)EGUI_THEME_PRIMARY.full, (int)egui_view_tab_bar_get_active_text_color(EGUI_VIEW_OF(&s_tab)).full);
}
...
static void test_tab_bar_get_active_text_color_after_set(void)
{
    ...
    EGUI_TEST_ASSERT_EQUAL_INT((int)c.full, (int)egui_view_tab_bar_get_active_text_color(EGUI_VIEW_OF(&s_tab)).full);
}
...
static void test_tab_bar_get_active_text_color_update(void)
{
    ...
    EGUI_TEST_ASSERT_EQUAL_INT((int)c1.full, (int)egui_view_tab_bar_get_active_text_color(EGUI_VIEW_OF(&s_tab)).full);
    s_tab.active_text_color = c2;
    EGUI_TEST_ASSERT_EQUAL_INT((int)c2.full, (int)egui_view_tab_bar_get_active_text_color(EGUI_VIEW_OF(&s_tab)).full);
}
