...
EGUI_BACKGROUND_COLOR_PARAM_INIT_SOLID(bg_page_1_param_normal, EGUI_COLOR_RED, EGUI_ALPHA_100);
...
static void egui_page_2_timer_callback(egui_timer_t *timer)
{
    egui_page_2_t *local = (egui_page_2_t *)timer->user_data;
    EGUI_UNUSED(local);
    EGUI_LOG_INF("egui_page_2_timer_callback\n");
...
void egui_page_2_on_open(egui_page_base_t *self)
{
    ...
    // start timer
    egui_page_base_start_timer(self, &local->timer, 3000, 3000);
    ...
    egui_view_label_set_text((egui_view_t *)&local->label_1, "egui_page_2");
...
void egui_page_2_on_close(egui_page_base_t *self)
{
    egui_page_2_t *local = (egui_page_2_t *)self;
    EGUI_UNUSED(local);
...
void egui_page_2_init(egui_page_base_t *self, egui_core_t *core)
{
    ...
    // update api.
    self->api = &EGUI_VIEW_API_TABLE_NAME(egui_page_2_t);

    // init local data.
    local->index = 0;
    egui_timer_init_timer(&local->timer, (void *)local, egui_page_2_timer_callback);

    egui_page_base_set_name(self, "egui_page_2");
}
