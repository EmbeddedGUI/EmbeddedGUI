#if defined(EGUI_DRIVER_LCD_GC9B71_ENABLE) && EGUI_DRIVER_LCD_GC9B71_ENABLE
...
/* GC9B71-specific commands (not in MIPI DCS common set) */
...
/* Color modes */
#define GC9B71_COLOR_MODE_16BIT 0x55 /* RGB565 */
...
static int gc9b71_init(egui_hal_lcd_driver_t *self, const egui_hal_lcd_config_t *config)
{
    ...
    {
        uint8_t colmod = GC9B71_COLOR_MODE_16BIT;
        self->io->tx_param(self->io, LCD_CMD_COLMOD, &colmod, 1);
    }

    /* Memory access control */
    {
        uint8_t madctl = GC9B71_MADCTL_MX | GC9B71_MADCTL_MY | GC9B71_MADCTL_RGB;
...
/* Driver: reset */
static int gc9b71_reset(egui_hal_lcd_driver_t *self)
{
...
/* Driver: del */
static void gc9b71_del(egui_hal_lcd_driver_t *self)
{
    if (self->set_rst)
    {
        self->set_rst(0);
...
/* Driver: draw_area */
static void gc9b71_draw_area(egui_hal_lcd_driver_t *self, int16_t x, int16_t y, int16_t w, int16_t h, const void *data, uint32_t len)
{
...
static void gc9b71_setup_driver(egui_hal_lcd_driver_t *driver, egui_panel_io_handle_t io, void (*set_rst)(uint8_t level))
{
    memset(driver, 0, sizeof(egui_hal_lcd_driver_t));

    driver->name = "GC9B71";

    driver->reset = gc9b71_reset;
    driver->init = gc9b71_init;
    driver->del = gc9b71_del;
    driver->draw_area = gc9b71_draw_area;
    driver->mirror = NULL;
    driver->swap_xy = NULL;
...
void egui_lcd_gc9b71_init(egui_hal_lcd_driver_t *storage, egui_panel_io_handle_t io, void (*set_rst)(uint8_t level))
{
