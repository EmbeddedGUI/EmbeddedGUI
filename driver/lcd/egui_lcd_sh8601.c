#if defined(EGUI_DRIVER_LCD_SH8601_ENABLE) && EGUI_DRIVER_LCD_SH8601_ENABLE

/**
 * @file egui_lcd_sh8601.c
 * @brief SH8601 LCD driver implementation
 *
...
static int sh8601_init(egui_hal_lcd_driver_t *self, const egui_hal_lcd_config_t *config)
{
    ...
    {
        uint8_t colmod = SH8601_COLOR_MODE_RGB565;
        self->io->tx_param(self->io, LCD_CMD_COLMOD, &colmod, 1);
    }

    /* Memory access control */
    {
        uint8_t madctl = SH8601_MADCTL_MX | SH8601_MADCTL_MY;
...
/* Driver: reset */
static int sh8601_reset(egui_hal_lcd_driver_t *self)
{
...
/* Driver: del */
static void sh8601_del(egui_hal_lcd_driver_t *self)
{
    if (self->set_rst)
    {
        self->set_rst(0);
...
static void sh8601_setup_driver(egui_hal_lcd_driver_t *driver, egui_panel_io_handle_t io, void (*set_rst)(uint8_t level))
{
    ...
    driver->reset = sh8601_reset;
    driver->init = sh8601_init;
    driver->del = sh8601_del;
    driver->draw_area = sh8601_draw_area;
    driver->mirror = NULL;
    driver->swap_xy = NULL;
...
void egui_lcd_sh8601_init(egui_hal_lcd_driver_t *storage, egui_panel_io_handle_t io, void (*set_rst)(uint8_t level))
{
    ...
    sh8601_setup_driver(storage, io, set_rst);
}

#endif /* EGUI_DRIVER_LCD_SH8601_ENABLE */
