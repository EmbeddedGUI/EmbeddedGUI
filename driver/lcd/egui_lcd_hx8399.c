#if defined(EGUI_DRIVER_LCD_HX8399_ENABLE) && EGUI_DRIVER_LCD_HX8399_ENABLE
...
static int hx8399_init(egui_hal_lcd_driver_t *self, const egui_hal_lcd_config_t *config)
{
    ...
    {
        uint8_t colmod = HX8399_COLOR_MODE_16BIT;
        if (config->color_depth == 24)
        {
            colmod = HX8399_COLOR_MODE_24BIT;
        }
        else if (config->color_depth == 18)
        {
            colmod = HX8399_COLOR_MODE_18BIT;
        }
    ...
    {
        uint8_t madctl = HX8399_MADCTL_MX | HX8399_MADCTL_BGR;
...
/* Driver: reset */
static int hx8399_reset(egui_hal_lcd_driver_t *self)
{
...
/* Driver: del */
static void hx8399_del(egui_hal_lcd_driver_t *self)
{
    if (self->set_rst)
    {
        self->set_rst(0);
...
static void hx8399_setup_driver(egui_hal_lcd_driver_t *driver, egui_panel_io_handle_t io, void (*set_rst)(uint8_t level))
{
    ...
    driver->reset = hx8399_reset;
    driver->init = hx8399_init;
    driver->del = hx8399_del;
    driver->draw_area = hx8399_draw_area;
    driver->mirror = NULL;
    driver->swap_xy = NULL;
...
void egui_lcd_hx8399_init(egui_hal_lcd_driver_t *storage, egui_panel_io_handle_t io, void (*set_rst)(uint8_t level))
{
    ...
    hx8399_setup_driver(storage, io, set_rst);
}

#endif /* EGUI_DRIVER_LCD_HX8399_ENABLE */
