#if defined(EGUI_DRIVER_LCD_JD9165_ENABLE) && EGUI_DRIVER_LCD_JD9165_ENABLE
...
/* JD9165-specific commands (not in MIPI DCS common set) */
...
static int jd9165_init(egui_hal_lcd_driver_t *self, const egui_hal_lcd_config_t *config)
{
    ...
    {
        uint8_t colmod = JD9165_COLOR_MODE_16BIT;
        self->io->tx_param(self->io, LCD_CMD_COLMOD, &colmod, 1);
    }

    /* Memory access control */
    {
        uint8_t madctl = JD9165_MADCTL_MX | JD9165_MADCTL_MY;
...
/* Driver: reset */
static int jd9165_reset(egui_hal_lcd_driver_t *self)
{
...
/* Driver: del */
static void jd9165_del(egui_hal_lcd_driver_t *self)
{
    if (self->set_rst)
    {
        self->set_rst(0);
...
/* Driver: draw_area */
static void jd9165_draw_area(egui_hal_lcd_driver_t *self, int16_t x, int16_t y, int16_t w, int16_t h, const void *data, uint32_t len)
{
...
static void jd9165_setup_driver(egui_hal_lcd_driver_t *driver, egui_panel_io_handle_t io, void (*set_rst)(uint8_t level))
{
    memset(driver, 0, sizeof(egui_hal_lcd_driver_t));

    driver->name = "JD9165";

    driver->reset = jd9165_reset;
    driver->init = jd9165_init;
    driver->del = jd9165_del;
    driver->draw_area = jd9165_draw_area;
    driver->mirror = NULL;
    driver->swap_xy = NULL;
...
void egui_lcd_jd9165_init(egui_hal_lcd_driver_t *storage, egui_panel_io_handle_t io, void (*set_rst)(uint8_t level))
{
    ...
    jd9165_setup_driver(storage, io, set_rst);
}

#endif /* EGUI_DRIVER_LCD_JD9165_ENABLE */
