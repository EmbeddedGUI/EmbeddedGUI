...
static void st7789_write_cmd(egui_hal_lcd_driver_t *self, uint8_t cmd)
{
...
static void st7789_write_data_byte(egui_hal_lcd_driver_t *self, uint8_t data)
{
    if (self->gpio && self->gpio->set_dc)
    {
...
static void st7789_write_data(egui_hal_lcd_driver_t *self, const uint8_t *data, uint32_t len)
{
    if (self->gpio && self->gpio->set_dc)
    {
        self->gpio->set_dc(1); /* Data mode */
...
static void st7789_hw_reset(egui_hal_lcd_driver_t *self)
{
...
static int st7789_init(egui_hal_lcd_driver_t *self, const egui_hal_lcd_config_t *config)
{
    ...
    /* Hardware reset */
    st7789_hw_reset(self);

    /* Software reset */
    st7789_write_cmd(self, ST7789_SWRESET);
    egui_api_delay(120); /* Wait 150ms */

    /* Sleep out */
    st7789_write_cmd(self, ST7789_SLPOUT);
    egui_api_delay(120); /* Wait 500ms */

    /* Set color mode to 16-bit RGB565 */
    st7789_write_cmd(self, ST7789_COLMOD);
    st7789_write_data_byte(self, ST7789_COLOR_MODE_16BIT);

    /* Set RAM control for little endian (optional, depends on MCU) */
    st7789_write_cmd(self, ST7789_RAMCTRL);
    {
        uint8_t data[] = {0x00, 0xF8}; /* Little endian */
        st7789_write_data(self, data, sizeof(data));
    }

    /* Set rotation (default 0) */
    st7789_write_cmd(self, ST7789_MADCTL);
    st7789_write_data_byte(self, ST7789_MADCTL_MX | ST7789_MADCTL_MY | ST7789_MADCTL_RGB);

    /* Inversion on (ST7789 typically needs this) */
    if (config->invert_color)
    {
        st7789_write_cmd(self, ST7789_INVON);
    }
    else
    {
        st7789_write_cmd(self, ST7789_INVOFF);
    }
    /* Display on */
    st7789_write_cmd(self, ST7789_DISPON);
...
/* Driver: deinit */
static void st7789_deinit(egui_hal_lcd_driver_t *self)
{
    /* Backlight off */
    if (self->set_brightness)
    {
    ...
    /* Display off */
    st7789_write_cmd(self, ST7789_DISPOFF);

    /* Sleep in */
    st7789_write_cmd(self, ST7789_SLPIN);
...
/* Driver: set_window */
static void st7789_set_window(egui_hal_lcd_driver_t *self, int16_t x, int16_t y, int16_t w, int16_t h)
{
    ...
    /* Column address set */
    st7789_write_cmd(self, ST7789_CASET);
    {
    ...
    /* Row address set */
    st7789_write_cmd(self, ST7789_RASET);
    {
    ...
    /* Write to RAM */
    st7789_write_cmd(self, ST7789_RAMWR);
}

/* Driver: write_pixels */
static void st7789_write_pixels(egui_hal_lcd_driver_t *self, const void *data, uint32_t len)
{
    if (self->gpio && self->gpio->set_dc)
    {
        self->gpio->set_dc(1); /* Data mode */
...
static void st7789_wait_dma_complete(egui_hal_lcd_driver_t *self)
{
    if (self->bus.spi->wait_complete)
    {
        self->bus.spi->wait_complete();
...
/* Driver: set_rotation */
static void st7789_set_rotation(egui_hal_lcd_driver_t *self, uint8_t rotation)
{
    ...
    st7789_write_cmd(self, ST7789_MADCTL);
    st7789_write_data_byte(self, madctl);
}
...
{
    st7789_write_cmd(self, invert ? ST7789_INVON : ST7789_INVOFF);
}

/* Internal: setup driver function pointers */
static void st7789_setup_driver(egui_hal_lcd_driver_t *driver, const egui_bus_spi_ops_t *spi, const egui_lcd_gpio_ops_t *gpio)
{
    ...
    driver->deinit = st7789_deinit;
    driver->set_window = st7789_set_window;
    driver->write_pixels = st7789_write_pixels;
    driver->wait_dma_complete = spi->wait_complete ? st7789_wait_dma_complete : NULL;
    driver->set_rotation = st7789_set_rotation;
...
void egui_lcd_st7789_init(egui_hal_lcd_driver_t *storage, const egui_bus_spi_ops_t *spi, const egui_lcd_gpio_ops_t *gpio)
{
    ...
    st7789_setup_driver(storage, spi, gpio);
}
