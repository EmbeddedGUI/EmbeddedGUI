#if defined(EGUI_DRIVER_LCD_EK9716B_ENABLE) && EGUI_DRIVER_LCD_EK9716B_ENABLE
...
/* EK9716B-specific commands (not in MIPI DCS common set) */
...
/* Color modes */
#define EK9716B_COLOR_MODE_16BIT 0x55 /* RGB565 */
...
static int ek9716b_init(egui_hal_lcd_driver_t *self, const egui_hal_lcd_config_t *config)
{
    ...
    {
        uint8_t colmod = EK9716B_COLOR_MODE_16BIT;
        self->io->tx_param(self->io, LCD_CMD_COLMOD, &colmod, 1);
    }

    /* Memory access control */
    {
        uint8_t madctl = EK9716B_MADCTL_MX | EK9716B_MADCTL_MY;
...
/* Driver: reset */
static int ek9716b_reset(egui_hal_lcd_driver_t *self)
{
...
/* Driver: del */
static void ek9716b_del(egui_hal_lcd_driver_t *self)
{
    if (self->set_rst)
    {
        self->set_rst(0);
...
/* Driver: draw_area */
static void ek9716b_draw_area(egui_hal_lcd_driver_t *self, int16_t x, int16_t y, int16_t w, int16_t h, const void *data, uint32_t len)
{
...
static void ek9716b_setup_driver(egui_hal_lcd_driver_t *driver, egui_panel_io_handle_t io, void (*set_rst)(uint8_t level))
{
    memset(driver, 0, sizeof(egui_hal_lcd_driver_t));

    driver->name = "EK9716B";

    driver->reset = ek9716b_reset;
    driver->init = ek9716b_init;
    driver->del = ek9716b_del;
    driver->draw_area = ek9716b_draw_area;
    driver->mirror = NULL;
    driver->swap_xy = NULL;
...
void egui_lcd_ek9716b_init(egui_hal_lcd_driver_t *storage, egui_panel_io_handle_t io, void (*set_rst)(uint8_t level))
{
    ...
    ek9716b_setup_driver(storage, io, set_rst);
}

#endif /* EGUI_DRIVER_LCD_EK9716B_ENABLE */
