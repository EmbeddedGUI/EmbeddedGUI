#if defined(EGUI_DRIVER_LCD_SPD2010_ENABLE) && EGUI_DRIVER_LCD_SPD2010_ENABLE

/**
 * @file egui_lcd_spd2010.c
 * @brief SPD2010 LCD driver implementation
 *
 * Uses unified Panel IO interface for all bus communication.
 */

#include "egui_lcd_spd2010.h"
#include "egui_lcd_common.h"
#include <string.h>
#include "core/egui_api.h"

/* SPD2010-specific commands (not in MIPI DCS common set) */

/* MADCTL bits */
#define SPD2010_MADCTL_MY  0x80
#define SPD2010_MADCTL_MX  0x40
#define SPD2010_MADCTL_MV  0x20
#define SPD2010_MADCTL_BGR 0x08

/* Color modes */
#define SPD2010_COLOR_MODE_16BIT 0x55 /* RGB565 */
#define SPD2010_COLOR_MODE_18BIT 0x66 /* RGB666 */
#define SPD2010_COLOR_MODE_24BIT 0x77 /* RGB888 */

/* Driver: init */
static int spd2010_init(egui_hal_lcd_driver_t *self, const egui_hal_lcd_config_t *config)
{
    /* Save config */
    memcpy(&self->config, config, sizeof(egui_hal_lcd_config_t));

    /* If custom init is provided, use it instead of default sequence */
    if (config->custom_init)
    {
        return config->custom_init(self->io, config);
    }

    /* Software reset */
    self->io->tx_param(self->io, LCD_CMD_SWRESET, NULL, 0);
    egui_api_delay(120);

    /* Sleep out */
    self->io->tx_param(self->io, LCD_CMD_SLPOUT, NULL, 0);
    egui_api_delay(120);

    /* Set color mode based on config */
    {
        uint8_t colmod = SPD2010_COLOR_MODE_16BIT;
        if (config->color_depth == 24)
        {
            colmod = SPD2010_COLOR_MODE_24BIT;
        }
        else if (config->color_depth == 18)
        {
            colmod = SPD2010_COLOR_MODE_18BIT;
        }
        self->io->tx_param(self->io, LCD_CMD_COLMOD, &colmod, 1);
    }

    /* Memory access control */
    {
        uint8_t madctl = SPD2010_MADCTL_MX | SPD2010_MADCTL_MY;
        self->io->tx_param(self->io, LCD_CMD_MADCTL, &madctl, 1);
    }

    /* Inversion control */
    if (config->invert_color)
    {
        self->io->tx_param(self->io, LCD_CMD_INVON, NULL, 0);
    }
    else
    {
        self->io->tx_param(self->io, LCD_CMD_INVOFF, NULL, 0);
    }

    /* Display on */
    self->io->tx_param(self->io, LCD_CMD_DISPON, NULL, 0);

    return 0;
}

/* Driver: reset */
static int spd2010_reset(egui_hal_lcd_driver_t *self)
{
    egui_lcd_hw_reset(self, 10);
    return 0;
}

/* Driver: del */
static void spd2010_del(egui_hal_lcd_driver_t *self)
{
    if (self->set_rst)
    {
        self->set_rst(0);
    }
    memset(self, 0, sizeof(egui_hal_lcd_driver_t));
}

/* Driver: draw_area */
static void spd2010_draw_area(egui_hal_lcd_driver_t *self, int16_t x, int16_t y, int16_t w, int16_t h, const void *data, uint32_t len)
{
    uint16_t x0 = x + self->config.x_offset;
    uint16_t y0 = y + self->config.y_offset;
    uint16_t x1 = x0 + w - 1;
    uint16_t y1 = y0 + h - 1;

    uint8_t caset[] = {x0 >> 8, x0 & 0xFF, x1 >> 8, x1 & 0xFF};
    self->io->tx_param(self->io, LCD_CMD_CASET, caset, sizeof(caset));

    uint8_t raset[] = {y0 >> 8, y0 & 0xFF, y1 >> 8, y1 & 0xFF};
    self->io->tx_param(self->io, LCD_CMD_RASET, raset, sizeof(raset));

    self->io->tx_color(self->io, LCD_CMD_RAMWR, data, len);
}

/* Driver: set_power */
static void spd2010_set_power(egui_hal_lcd_driver_t *self, uint8_t on)
{
    if (on)
    {
        self->io->tx_param(self->io, LCD_CMD_SLPOUT, NULL, 0);
        egui_api_delay(120);
        self->io->tx_param(self->io, LCD_CMD_DISPON, NULL, 0);
    }
    else
    {
        self->io->tx_param(self->io, LCD_CMD_DISPOFF, NULL, 0);
        self->io->tx_param(self->io, LCD_CMD_SLPIN, NULL, 0);
    }
}

/* Driver: set_invert */
static void spd2010_set_invert(egui_hal_lcd_driver_t *self, uint8_t invert)
{
    self->io->tx_param(self->io, invert ? LCD_CMD_INVON : LCD_CMD_INVOFF, NULL, 0);
}

/* Internal: setup driver function pointers */
static void spd2010_setup_driver(egui_hal_lcd_driver_t *driver, egui_panel_io_handle_t io, void (*set_rst)(uint8_t level))
{
    memset(driver, 0, sizeof(egui_hal_lcd_driver_t));

    driver->name = "SPD2010";

    driver->reset = spd2010_reset;
    driver->init = spd2010_init;
    driver->del = spd2010_del;
    driver->draw_area = spd2010_draw_area;
    driver->mirror = NULL;
    driver->swap_xy = NULL;
    driver->set_power = spd2010_set_power;
    driver->set_invert = spd2010_set_invert;

    driver->io = io;
    driver->set_rst = set_rst;
}

/* Public: init (static allocation) */
void egui_lcd_spd2010_init(egui_hal_lcd_driver_t *storage, egui_panel_io_handle_t io, void (*set_rst)(uint8_t level))
{
    if (!storage || !io || !io->tx_param)
    {
        return;
    }

    spd2010_setup_driver(storage, io, set_rst);
}

#endif /* EGUI_DRIVER_LCD_SPD2010_ENABLE */
