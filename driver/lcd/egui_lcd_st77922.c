#if defined(EGUI_DRIVER_LCD_ST77922_ENABLE) && EGUI_DRIVER_LCD_ST77922_ENABLE
...
/* ST77922-specific commands (not in MIPI DCS common set) */
...
/* Color modes */
#define ST77922_COLOR_MODE_16BIT 0x55 /* RGB565 */
...
static int st77922_init(egui_hal_lcd_driver_t *self, const egui_hal_lcd_config_t *config)
{
    ...
    {
        uint8_t colmod = ST77922_COLOR_MODE_16BIT;
        self->io->tx_param(self->io, LCD_CMD_COLMOD, &colmod, 1);
    }

    /* Memory access control */
    {
        uint8_t madctl = ST77922_MADCTL_MX | ST77922_MADCTL_MY;
...
/* Driver: reset */
static int st77922_reset(egui_hal_lcd_driver_t *self)
{
...
/* Driver: del */
static void st77922_del(egui_hal_lcd_driver_t *self)
{
    if (self->set_rst)
    {
        self->set_rst(0);
...
/* Driver: draw_area */
static void st77922_draw_area(egui_hal_lcd_driver_t *self, int16_t x, int16_t y, int16_t w, int16_t h, const void *data, uint32_t len)
{
...
static void st77922_setup_driver(egui_hal_lcd_driver_t *driver, egui_panel_io_handle_t io, void (*set_rst)(uint8_t level))
{
    memset(driver, 0, sizeof(egui_hal_lcd_driver_t));

    driver->name = "ST77922";

    driver->reset = st77922_reset;
    driver->init = st77922_init;
    driver->del = st77922_del;
    driver->draw_area = st77922_draw_area;
    driver->mirror = NULL;
    driver->swap_xy = NULL;
...
void egui_lcd_st77922_init(egui_hal_lcd_driver_t *storage, egui_panel_io_handle_t io, void (*set_rst)(uint8_t level))
{
