#if defined(EGUI_DRIVER_LCD_ST7262_ENABLE) && EGUI_DRIVER_LCD_ST7262_ENABLE
...
/* ST7262-specific commands (not in MIPI DCS common set) */
...
static int st7262_init(egui_hal_lcd_driver_t *self, const egui_hal_lcd_config_t *config)
{
    ...
    {
        uint8_t colmod = ST7262_COLOR_MODE_16BIT;
        self->io->tx_param(self->io, LCD_CMD_COLMOD, &colmod, 1);
    }

    /* Memory access control */
    {
        uint8_t madctl = ST7262_MADCTL_MX | ST7262_MADCTL_MY;
...
/* Driver: reset */
static int st7262_reset(egui_hal_lcd_driver_t *self)
{
...
/* Driver: del */
static void st7262_del(egui_hal_lcd_driver_t *self)
{
    if (self->set_rst)
    {
        self->set_rst(0);
...
/* Driver: draw_area */
static void st7262_draw_area(egui_hal_lcd_driver_t *self, int16_t x, int16_t y, int16_t w, int16_t h, const void *data, uint32_t len)
{
...
static void st7262_setup_driver(egui_hal_lcd_driver_t *driver, egui_panel_io_handle_t io, void (*set_rst)(uint8_t level))
{
    memset(driver, 0, sizeof(egui_hal_lcd_driver_t));

    driver->name = "ST7262";

    driver->reset = st7262_reset;
    driver->init = st7262_init;
    driver->del = st7262_del;
    driver->draw_area = st7262_draw_area;
    driver->mirror = NULL;
    driver->swap_xy = NULL;
