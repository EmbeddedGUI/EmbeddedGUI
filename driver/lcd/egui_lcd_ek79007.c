#if defined(EGUI_DRIVER_LCD_EK79007_ENABLE) && EGUI_DRIVER_LCD_EK79007_ENABLE
...
/* EK79007-specific commands (not in MIPI DCS common set) */
...
static int ek79007_init(egui_hal_lcd_driver_t *self, const egui_hal_lcd_config_t *config)
{
    ...
    {
        uint8_t colmod = EK79007_COLOR_MODE_16BIT;
        if (config->color_depth == 24)
        {
            colmod = EK79007_COLOR_MODE_24BIT;
        }
        else if (config->color_depth == 18)
        {
            colmod = EK79007_COLOR_MODE_18BIT;
        }
    ...
    {
        uint8_t madctl = EK79007_MADCTL_MX | EK79007_MADCTL_MY;
...
/* Driver: reset */
static int ek79007_reset(egui_hal_lcd_driver_t *self)
{
...
/* Driver: del */
static void ek79007_del(egui_hal_lcd_driver_t *self)
{
    if (self->set_rst)
    {
        self->set_rst(0);
...
static void ek79007_setup_driver(egui_hal_lcd_driver_t *driver, egui_panel_io_handle_t io, void (*set_rst)(uint8_t level))
{
    ...
    driver->reset = ek79007_reset;
    driver->init = ek79007_init;
    driver->del = ek79007_del;
    driver->draw_area = ek79007_draw_area;
    driver->mirror = NULL;
    driver->swap_xy = NULL;
