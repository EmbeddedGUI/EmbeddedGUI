#if defined(EGUI_DRIVER_LCD_AXS15231B_ENABLE) && EGUI_DRIVER_LCD_AXS15231B_ENABLE
...
/* AXS15231B-specific commands (not in MIPI DCS common set) */
...
static int axs15231b_init(egui_hal_lcd_driver_t *self, const egui_hal_lcd_config_t *config)
{
    ...
    {
        uint8_t colmod = AXS15231B_COLOR_MODE_16BIT;
        self->io->tx_param(self->io, LCD_CMD_COLMOD, &colmod, 1);
    }

    /* Memory access control */
    {
        uint8_t madctl = AXS15231B_MADCTL_MX | AXS15231B_MADCTL_MY;
...
/* Driver: reset */
static int axs15231b_reset(egui_hal_lcd_driver_t *self)
{
...
/* Driver: del */
static void axs15231b_del(egui_hal_lcd_driver_t *self)
{
    if (self->set_rst)
    {
        self->set_rst(0);
...
static void axs15231b_setup_driver(egui_hal_lcd_driver_t *driver, egui_panel_io_handle_t io, void (*set_rst)(uint8_t level))
{
    ...
    driver->reset = axs15231b_reset;
    driver->init = axs15231b_init;
    driver->del = axs15231b_del;
    driver->draw_area = axs15231b_draw_area;
    driver->mirror = NULL;
    driver->swap_xy = NULL;
