#if defined(EGUI_DRIVER_TOUCH_ST7123_ENABLE) && EGUI_DRIVER_TOUCH_ST7123_ENABLE
...
/* Driver: del */
static void st7123_del(egui_hal_touch_driver_t *self)
{
    if (self->set_rst)
    {
        self->set_rst(0);
                                ...
                                uint8_t (*get_int)(void))
{
    ...
    driver->reset = st7123_reset;
    driver->init = st7123_init;
    driver->del = st7123_del;
                            ...
                            uint8_t (*get_int)(void))
{
    ...
    st7123_setup_driver(storage, io, set_rst, set_int, get_int);
}

#endif /* EGUI_DRIVER_TOUCH_ST7123_ENABLE */
