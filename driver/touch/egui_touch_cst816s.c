#if defined(EGUI_DRIVER_TOUCH_CST816S_ENABLE) && EGUI_DRIVER_TOUCH_CST816S_ENABLE
...
/* Helper: reset */
static int cst816s_reset(egui_hal_touch_driver_t *self)
{
    if (self->set_rst)
    {
        self->set_rst(0);
...
/* Driver: del */
static void cst816s_del(egui_hal_touch_driver_t *self)
{
    if (self->set_rst)
    {
        self->set_rst(0);
...
/* Driver: read */
static int cst816s_read(egui_hal_touch_driver_t *self, egui_core_t *core, egui_hal_touch_data_t *data)
{
    EGUI_UNUSED(core);
...
static void cst816s_setup_driver(egui_hal_touch_driver_t *driver, egui_panel_io_handle_t io, void (*set_rst)(uint8_t level), void (*set_int)(uint8_t level),
                                 uint8_t (*get_int)(void))
{
    ...
    driver->reset = cst816s_reset;
    driver->init = cst816s_init;
    driver->del = cst816s_del;
...
void egui_touch_cst816s_init(egui_hal_touch_driver_t *storage, egui_panel_io_handle_t io, void (*set_rst)(uint8_t level), void (*set_int)(uint8_t level),
                             uint8_t (*get_int)(void))
{
