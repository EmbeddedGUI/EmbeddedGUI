...
/*                                                                       */
static void gray_render_line(RAS_ARG_ TPos from_x, TPos from_y, TPos to_x, TPos to_y)
{
...
static int gray_clip_flags(RAS_ARG_ TPos x, TPos y)
{
    return ((x > ras.clip_box.xMax) << 0) | ((y > ras.clip_box.yMax) << 1) | ((x < ras.clip_box.xMin) << 2) | ((y < ras.clip_box.yMin) << 3);
}

static int gray_clip_vflags(RAS_ARG_ TPos y)
{
    return ((y > ras.clip_box.yMax) << 1) | ((y < ras.clip_box.yMin) << 3);
}

static void gray_vline(RAS_ARG_ TPos x1, TPos y1, TPos x2, TPos y2, int f1, int f2)
{
    f1 &= 10;
    f2 &= 10;
...
static void gray_line_to(RAS_ARG_ TPos x2, TPos y2)
{
    if (!ras.clipping)
    {
