...
/**
 * Enable SVG <image> element support in the runtime SVG renderer.
 * When disabled, embedded PNG/JPEG data URLs are ignored, allowing
 * applications that
 * only use vector SVG content to drop image decoders.
 */
...
/**
 * Enable SVG linearGradient/radialGradient paint support.
 * Applications that only render solid-color vector SVGs can disable this
 * to drop gradient
 * parsing and blending code from runtime SVG builds.
 */
...
/**
 * Enable SVG elliptical arc path commands and PlutoVG arc helpers.
 * Disabling this keeps path commands used by simple solid SVGs but renders
 * arc
 * commands as straight segments.
 */
...
/**
 * Enable path length accumulation in PlutoVG path extents.
 * Runtime SVG rendering only needs extents; applications can disable this
 * when they do not
 * call path length APIs from the same build.
 */
...
/**
 * Use a compact sine/cosine approximation in PlutoVG transform rotation.
 * Disabled by default to keep full SVG accuracy; size-focused demos can
 * enable
 * it when small rotation error is acceptable.
 */
...
/**
 * Enable SVG skewX/skewY transforms.
 */
...
/**
 * Enable SVG percentage radius resolution using the normalized viewport
 * diagonal. Size-focused demos that do not use percentage radii can disable it.
...
/**
 * Restrict SVG color parsing to hexadecimal colors only.
 * Disabled by default; enables full named/rgb/hsl color support.
 */
...
/**
 * Enable SVG stroke-dasharray/stroke-dashoffset support.
 * Applications that only render solid strokes can disable this to drop
 * dashed stroke parsing
 * and path cloning code.
 */
...
/**
 * Use a compact approximation for transformed stroke width scaling.
 * Disabled by default to keep PlutoVG stroke metrics closest to SVG spec;
 *
 * size-focused demos can enable it when small stroke-width error is acceptable.
 */
