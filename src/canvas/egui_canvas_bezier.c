                                  ...
                                  egui_dim_t stroke_width, egui_color_t color, egui_alpha_t alpha, int depth)
{
    ...
    if (dist_sq <= flatness_thresh || depth >= EGUI_CANVAS_BEZIER_MAX_DEPTH)
    {
        ...
        if (stroke_width > 1)
        {
            egui_canvas_draw_line_hq(self, x0, y0, x1, y1, stroke_width, color, alpha);
        }
        else
        {
            egui_canvas_draw_line(self, x0, y0, x1, y1, stroke_width, color, alpha);
        }
        return;
    }
    ...
    bezier_quad_recursive(self, x0, y0, mid01_x, mid01_y, mid_x, mid_y, stroke_width, color, alpha, depth + 1);
    bezier_quad_recursive(self, mid_x, mid_y, mid12_x, mid12_y, x1, y1, stroke_width, color, alpha, depth + 1);
}
                                  ...
                                  egui_dim_t stroke_width, egui_color_t color, egui_alpha_t alpha)
{
    ...
    bezier_quad_recursive(self, x0, y0, cx, cy, x1, y1, stroke_width, color, alpha, 0);
}
                                   ...
                                   egui_dim_t x1, egui_dim_t y1, egui_dim_t stroke_width, egui_color_t color, egui_alpha_t alpha, int depth)
{
    ...
    bezier_cubic_recursive(self, x0, y0, m01_x, m01_y, m012_x, m012_y, mid_x, mid_y, stroke_width, color, alpha, depth + 1);
    bezier_cubic_recursive(self, mid_x, mid_y, m123_x, m123_y, m23_x, m23_y, x1, y1, stroke_width, color, alpha, depth + 1);
}
                                   ...
                                   egui_dim_t x1, egui_dim_t y1, egui_dim_t stroke_width, egui_color_t color, egui_alpha_t alpha)
{
    ...
    bezier_cubic_recursive(self, x0, y0, cx0, cy0, cx1, cy1, x1, y1, stroke_width, color, alpha, 0);
}
