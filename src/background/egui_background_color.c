...
void egui_background_color_on_draw(egui_background_t *self, egui_region_t *region, const void *param)
{
    ...
    case EGUI_BACKGROUND_COLOR_TYPE_ROUND_RECTANGLE:
        if (color_param->stroke_width == 0)
        {
            egui_canvas_draw_round_rectangle_fill(0, 0, region->size.width, region->size.height, color_param->round_rectangle.radius,
                                                  color_param->color, color_param->alpha);
        }
        else
        {
            if (color_param->alpha != EGUI_ALPHA_0)
            {
                egui_canvas_draw_round_rectangle_fill(color_param->stroke_width / 2, color_param->stroke_width / 2,
                                                      region->size.width - color_param->stroke_width, region->size.height - color_param->stroke_width,
                                                      color_param->round_rectangle.radius, color_param->color, color_param->alpha);
            }
            egui_canvas_draw_round_rectangle(0, 0, region->size.width, region->size.height, color_param->round_rectangle.radius,
    ...
    case EGUI_BACKGROUND_COLOR_TYPE_ROUND_RECTANGLE_CORNERS:
        if (color_param->stroke_width == 0)
        {
            egui_canvas_draw_round_rectangle_corners_fill(
                    0, 0, region->size.width, region->size.height, color_param->round_rectangle_corners.radius_left_top,
                    color_param->round_rectangle_corners.radius_left_bottom, color_param->round_rectangle_corners.radius_right_top,
                    color_param->round_rectangle_corners.radius_right_top, color_param->color, color_param->alpha);
        }
        else
        {
            if (color_param->alpha != EGUI_ALPHA_0)
            {
                egui_canvas_draw_round_rectangle_corners_fill(
                        ...
                        region->size.height - color_param->stroke_width, color_param->round_rectangle_corners.radius_left_top - color_param->stroke_width / 2,
                        color_param->round_rectangle_corners.radius_left_bottom - color_param->stroke_width / 2,
                        color_param->round_rectangle_corners.radius_right_top - color_param->stroke_width / 2,
                        color_param->round_rectangle_corners.radius_right_top - color_param->stroke_width / 2, color_param->color, color_param->alpha);
            }
            egui_canvas_draw_round_rectangle_corners(
                    0, 0, region->size.width, region->size.height, color_param->round_rectangle_corners.radius_left_top,
                    color_param->round_rectangle_corners.radius_left_bottom, color_param->round_rectangle_corners.radius_right_top,
                    color_param->round_rectangle_corners.radius_right_top, color_param->stroke_width, color_param->stroke_color, color_param->stroke_alpha);
        }
        break;
    case EGUI_BACKGROUND_COLOR_TYPE_CIRCLE:
        if (color_param->stroke_width == 0)
        {
            egui_canvas_draw_circle_fill((region->size.width >> 1), (region->size.height >> 1), color_param->circle.radius, color_param->color,
                                         color_param->alpha);
        }
        else
        {
            if (color_param->alpha != EGUI_ALPHA_0)
            {
                egui_canvas_draw_circle_fill((region->size.width >> 1), (region->size.height >> 1),
                                             color_param->circle.radius - color_param->stroke_width / 2, color_param->color, color_param->alpha);
            }
            egui_canvas_draw_circle((region->size.width >> 1), (region->size.height >> 1), color_param->circle.radius, color_param->stroke_width,
