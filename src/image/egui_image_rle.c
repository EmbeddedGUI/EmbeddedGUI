...
#if !EGUI_CONFIG_IMAGE_RLE_EXTERNAL_WINDOW_PERSISTENT_CACHE_ENABLE && !EGUI_IMAGE_RLE_EXTERNAL_CACHE_SHARE_WINDOW
#error "EGUI_CONFIG_IMAGE_RLE_EXTERNAL_WINDOW_PERSISTENT_CACHE_ENABLE=0 requires EGUI_IMAGE_RLE_EXTERNAL_CACHE_SHARE_WINDOW"
#endif
...
static void egui_image_rle_draw_image(const egui_image_t *self, egui_dim_t x, egui_dim_t y)
{
    ...
    for (egui_dim_t row = img_y_start; row < img_y_end; row++)
    {
        ...
        if (use_row_cache)
        {
            ...
            pixel_buf = egui_image_decode_cache_pixel_row(row_in_band, draw_info->width, data_blk_size);
