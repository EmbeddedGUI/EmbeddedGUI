...
static uint8_t egui_view_virtual_stage_array_adapter_get_desc(void *adapter_context, uint32_t index, egui_virtual_stage_node_desc_t *out_desc)
{
    ...
    egui_api_memcpy(out_desc, item_ptr, (int)sizeof(*out_desc));
    return 1;
}
...
static void egui_view_virtual_stage_reset_slot(egui_view_virtual_stage_slot_t *slot)
{
    ...
    slot->view = NULL;
    egui_api_memset(&slot->render_region, 0, (int)sizeof(slot->render_region));
    egui_api_memset(&slot->bound_desc, 0, (int)sizeof(slot->bound_desc));
    slot->last_used_seq = 0;
    slot->needs_bind = 0;
    slot->needs_layout = 0;
    egui_api_memset(&slot->touch_state, 0, (int)sizeof(slot->touch_state));
}
...
static void egui_view_virtual_stage_release_cache(egui_view_virtual_stage_t *local)
{
    ...
    {
        egui_free(EGUI_VIEW_OF(local)->core, local->node_cache);
        local->node_cache = NULL;
    }
    if (local->draw_order != NULL)
    {
        egui_free(EGUI_VIEW_OF(local)->core, local->draw_order);
...
static uint8_t egui_view_virtual_stage_ensure_cache_capacity(egui_view_virtual_stage_t *local, uint32_t target_capacity)
{
    ...
    nodes = (egui_view_virtual_stage_cached_node_t *)egui_malloc(EGUI_VIEW_OF(local)->core, (int)(sizeof(*nodes) * target_capacity));
    if (nodes == NULL)
    {
        return 0;
    }

    draw_order = (uint32_t *)egui_malloc(EGUI_VIEW_OF(local)->core, (int)(sizeof(*draw_order) * target_capacity));
    if (draw_order == NULL)
    {
        egui_free(EGUI_VIEW_OF(local)->core, nodes);
        return 0;
    }
...
static void egui_view_virtual_stage_release_pins(egui_view_virtual_stage_t *local)
{
    ...
    {
        egui_free(EGUI_VIEW_OF(local)->core, local->pin_entries);
...
static uint8_t egui_view_virtual_stage_reload_cache(egui_view_virtual_stage_t *local)
{
    ...
    {
        egui_virtual_stage_node_desc_t desc;
        egui_api_memset(&desc, 0, (int)sizeof(desc));
...
static uint8_t egui_view_virtual_stage_ensure_pin_capacity(egui_view_virtual_stage_t *local, uint16_t target_count)
{
    ...
    pins = (egui_view_virtual_stage_pin_entry_t *)egui_malloc(EGUI_VIEW_OF(local)->core, (int)(sizeof(egui_view_virtual_stage_pin_entry_t) * new_capacity));
    if (pins == NULL)
    {
        return 0;
    }
    ...
    {
        egui_api_memcpy(pins, local->pin_entries, (int)(sizeof(egui_view_virtual_stage_pin_entry_t) * local->pin_count));
        egui_free(EGUI_VIEW_OF(local)->core, local->pin_entries);
    }
...
static void egui_view_virtual_stage_cancel_slot_capture(egui_view_virtual_stage_t *local, egui_view_virtual_stage_slot_t *slot)
{
    ...
    egui_api_memset(&cancel_event, 0, (int)sizeof(cancel_event));
...
static uint8_t egui_view_virtual_stage_refresh_cached_node(egui_view_t *self, egui_view_virtual_stage_t *local, uint32_t stable_id, uint8_t force_layout)
{
    ...
    old_desc = node->desc;
    egui_api_memset(&new_desc, 0, (int)sizeof(new_desc));
...
static int egui_view_virtual_stage_dispatch_touch_to_slot(egui_view_virtual_stage_t *local, egui_view_virtual_stage_slot_t *slot, egui_motion_event_t *event)
{
    ...
    core = egui_view_get_core(EGUI_VIEW_OF(local));
    slot->last_used_seq = ++local->slot_use_seq;
    egui_api_memset(&outer_touch_state, 0, (int)sizeof(outer_touch_state));
    egui_view_group_touch_state_exchange(core, &outer_touch_state);
    egui_view_group_touch_state_exchange(core, &slot->touch_state);
    {
...
{
    egui_view_virtual_stage_init(self, core);
                                               ...
                                               const egui_view_virtual_stage_array_setup_t *setup)
{
    ...
    egui_api_memset(&bridge_setup, 0, (int)sizeof(bridge_setup));
    if (setup != NULL)
    {
                                                ...
                                                const egui_view_virtual_stage_array_ops_t *ops, void *user_context)
{
    ...
    egui_api_memset(adapter, 0, (int)sizeof(*adapter));
