...
static void egui_view_virtual_list_reset_entry(egui_view_virtual_list_entry_t *entry)
{
    if (entry == NULL)
    {
        return;
...
static uint8_t egui_view_virtual_list_resolve_item_by_index_internal(egui_view_t *self, uint32_t index, egui_view_virtual_list_entry_t *entry)
{
    ...
    egui_view_virtual_list_reset_entry(entry);
    ...
    {
        entry->index = index;
        entry->stable_id = adapter->get_stable_id != NULL ? adapter->get_stable_id(adapter_context, index) : egui_view_virtual_list_default_stable_id(index);
    }

    return 1;
}

static const egui_view_virtual_list_slot_t *egui_view_virtual_list_find_slot_by_view(egui_view_virtual_list_t *local, egui_view_t *view)
{
    uint8_t slot_index;

    for (slot_index = 0; slot_index < EGUI_VIEW_VIRTUAL_VIEWPORT_MAX_SLOTS; slot_index++)
    {
        const egui_view_virtual_list_slot_t *slot = &local->base.slots[slot_index];
...
static int32_t egui_view_virtual_list_find_slot_index_by_stable_id_internal(egui_view_virtual_list_t *local, uint32_t stable_id)
{
    uint8_t slot_index;
...
static uint8_t egui_view_virtual_list_resolve_slot_internal(egui_view_t *self, const egui_view_virtual_list_slot_t *slot, egui_view_virtual_list_entry_t *entry)
{
    ...
    if (egui_view_virtual_list_resolve_item_by_index_internal(self, slot->index, entry) && entry->stable_id == slot->stable_id)
    {
        return 1;
    }

    return egui_view_virtual_list_resolve_item_by_stable_id(self, slot->stable_id, entry);
}
...
void egui_view_virtual_list_apply_setup(egui_view_t *self, const egui_view_virtual_list_setup_t *setup)
{
    ...
    {
        egui_view_virtual_list_apply_params(self, setup->params);
    }

    egui_view_virtual_list_set_data_source(self, setup->data_source, setup->data_source_context);
    egui_view_virtual_list_set_state_cache_limits(self, setup->state_cache_max_entries, setup->state_cache_max_bytes);
}

void egui_view_virtual_list_init_with_setup(egui_view_t *self, const egui_view_virtual_list_setup_t *setup)
{
    egui_view_virtual_list_init(self);
    egui_view_virtual_list_apply_setup(self, setup);
}
...
{
    int32_t index = egui_view_virtual_list_find_index_by_stable_id(self, stable_id);
    ...
    return egui_view_virtual_list_resolve_item_by_index_internal(self, (uint32_t)index, entry);
}
...
{
    EGUI_LOCAL_INIT(egui_view_virtual_list_t);
    const egui_view_virtual_list_slot_t *slot = egui_view_virtual_list_find_slot_by_view(local, item_view);

    return egui_view_virtual_list_resolve_slot_internal(self, slot, entry);
}
...
int32_t egui_view_virtual_list_get_item_y_by_stable_id(egui_view_t *self, uint32_t stable_id)
{
    ...
    return egui_view_virtual_list_get_item_y(self, (uint32_t)index);
}
...
int32_t egui_view_virtual_list_get_item_height_by_stable_id(egui_view_t *self, uint32_t stable_id)
{
    ...
    return egui_view_virtual_list_get_item_height(self, (uint32_t)index);
}
...
uint8_t egui_view_virtual_list_ensure_item_visible_by_stable_id(egui_view_t *self, uint32_t stable_id, int32_t inset)
{
    ...
    if (stable_id == EGUI_VIEW_VIRTUAL_VIEWPORT_INVALID_ID || egui_view_virtual_list_find_index_by_stable_id(self, stable_id) < 0)
    {
        return 0;
    }

    slot = egui_view_virtual_list_find_slot_by_stable_id(self, stable_id);
    if (egui_view_virtual_viewport_is_slot_fully_visible(self, slot, inset))
    {
        return 1;
    }

    egui_view_virtual_list_scroll_to_stable_id(self, stable_id, inset);
    return 1;
}
...
{
    EGUI_LOCAL_INIT(egui_view_virtual_list_t);
    return egui_view_virtual_list_find_slot_index_by_stable_id_internal(local, stable_id);
}

const egui_view_virtual_list_slot_t *egui_view_virtual_list_find_slot_by_stable_id(egui_view_t *self, uint32_t stable_id)
{
    int32_t slot_index = egui_view_virtual_list_find_slot_index_by_stable_id(self, stable_id);

    return slot_index >= 0 ? egui_view_virtual_list_get_slot(self, (uint8_t)slot_index) : NULL;
}

egui_view_t *egui_view_virtual_list_find_view_by_stable_id(egui_view_t *self, uint32_t stable_id)
{
    const egui_view_virtual_list_slot_t *slot = egui_view_virtual_list_find_slot_by_stable_id(self, stable_id);
...
{
    const egui_view_virtual_list_slot_t *slot = egui_view_virtual_list_get_slot(self, slot_index);
    return egui_view_virtual_list_resolve_slot_internal(self, slot, entry);
}
...
uint8_t egui_view_virtual_list_visit_visible_items(egui_view_t *self, egui_view_virtual_list_visible_item_visitor_t visitor, void *context)
{
    ...
    slot_count = egui_view_virtual_list_get_slot_count(self);
    for (slot_index = 0; slot_index < slot_count; slot_index++)
    {
        ...
        if (!egui_view_virtual_list_get_slot_entry(self, slot_index, &entry))
        {
            continue;
        }
...
typedef struct egui_view_virtual_list_find_visible_item_context
{
    egui_view_virtual_list_visible_item_matcher_t matcher;
    void *context;
    uint32_t best_index;
    egui_view_t *best_view;
    egui_view_virtual_list_entry_t best_entry;
} egui_view_virtual_list_find_visible_item_context_t;

static uint8_t egui_view_virtual_list_find_visible_item_visitor(egui_view_t *self, const egui_view_virtual_list_slot_t *slot,
                                                                const egui_view_virtual_list_entry_t *entry, egui_view_t *item_view, void *context)
{
    egui_view_virtual_list_find_visible_item_context_t *ctx = (egui_view_virtual_list_find_visible_item_context_t *)context;
...
{
    egui_view_virtual_list_find_visible_item_context_t find_ctx = {
            .matcher = matcher,
            .context = context,
    ...
    egui_view_virtual_list_visit_visible_items(self, egui_view_virtual_list_find_visible_item_visitor, &find_ctx);
