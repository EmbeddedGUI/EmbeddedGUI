...
```c
// 默认配置（保留基础 fast draw，不启用 ASCII 直查表）
#define EGUI_CONFIG_FUNCTION_FONT_STD_FAST_DRAW 1
// 额外 ASCII cache RAM 占用：0 B

// 启用 ASCII 直查表（适合纯英文 UI）
#define EGUI_CONFIG_FUNCTION_FONT_STD_FAST_DRAW 2
// RAM 占用：268 B heap + allocator overhead；egui_core_t 理论上增加 4 B
```
...
**使用建议**：
- 默认保持 `1` 以保留基础 fast draw；只有 ASCII 直查表默认关闭
- 即使禁用，系统仍有多级优化缓存（last_code、block、相邻字符），性能已经很好
- 仅在纯英文 UI 且追求极致性能时切到 `2`
- 中文 UI 或低 RAM 场景无需开启

##### ASCII Lookup Index 8-bit（~128 B heap）

将 ASCII lookup 索引从 uint16_t 降为 uint8_t（需配合 `EGUI_CONFIG_FUNCTION_FONT_STD_FAST_DRAW=2`）。
...
**使用建议**：
- 当前 public 面没有单独开关
- 只有在已经决定切到 `EGUI_CONFIG_FUNCTION_FONT_STD_FAST_DRAW=2` 且明确需要继续压缩该 cache 时，才值得在项目分支里定制实现
...
- [ ] PFB 大小适配 RAM 预算
- [ ] 多页面使用 union 共享内存
- [ ] 运行 `python scripts/size_analysis/main.py --case-set typical` 确认优化效果
