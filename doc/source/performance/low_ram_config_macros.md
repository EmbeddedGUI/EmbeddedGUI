...
> `2026-04-06` 起，这个名字不再是当前框架 public 配置入口；当前主线已把它内收为 `src/image/egui_image_std.c` 里的实现私有常量，默认行为固定为 `4`。下面保留的是历史 A/B 记录，便于说明为什么它不再值得暴露成公共宏。

| 属性 | 值 |
|------|---|
| 默认值 | 当前实现内固定 `4` |
| HelloPerformance | 当前随实现内固定 `4`（历史 public 实验值 `0`） |
| RAM 类型 | BSS |
| HP 节省 | ~33 B |
...
因此这颗宏当前默认继续保持 `4`。但它的收益也只有 `text -156B, bss -40B`，远低于当前“单颗 public 宏至少要值得 >1KB code-size 回收”的门槛；同时 `0` 路径在当前主线仍会带来 `8` 帧真实渲染差异，所以这个名字已不再保留为 public 宏入口，只保留历史 A/B 记录。
...
**位置：** [src/config/egui_config_default.h](../../../src/config/egui_config_default.h)
...
**位置：** [src/image/egui_image_decode_utils.h](../../../src/image/egui_image_decode_utils.h)，使用处 [src/image/egui_image_decode_utils.c](../../../src/image/egui_image_decode_utils.c)

> `2026-04-06` 起，这个名字不再是当前框架 public 配置入口；当前主线已把它内收为实现私有常量，默认行为固定为 `0`。下面保留的是历史 A/B 记录，便于说明为什么它不再值得暴露成公共宏。

| 属性 | 值 |
|------|---|
| 默认值 | 当前实现内固定 `0` |
| HelloPerformance | 当前随实现内固定 `0`（历史 public 实验值 `1`） |
| RAM 类型 | heap（节省独立分配） |
| HP 节省 | ~248 B（240B data + 8B 指针） |
| 依赖 | 需 `EGUI_CONFIG_FUNCTION_IMAGE_CODEC_FAST_DRAW>=1` |
...
因此这颗宏当前不应再写成 `HelloPerformance=1` 的 active override。对 `HelloPerformance` 而言，`1` 现阶段更适合作为行为等价、收益很小的历史条件实验值；当前 shipped/default 继续记为 `0`。由于它最多也只是在 `text -132B, bss -8B` 量级做取舍，远低于当前“单颗 public 宏至少要值得 >1KB code-size 回收”的门槛，这个名字已不再保留为 public 宏入口。

---

### 4. `EGUI_CONFIG_FUNCTION_IMAGE_CODEC_FAST_DRAW`

**位置：** [src/image/egui_image_qoi.c](../../../src/image/egui_image_qoi.c)，[src/image/egui_image_rle.c](../../../src/image/egui_image_rle.c)

补充：`2026-04-06` 起，`EGUI_CONFIG_IMAGE_CODEC_TAIL_ROW_CACHE_MAX_COLS` 已内收为实现私有固定值 `0`。历史把 tail cap 压到 `184/176/144/96` 的 A/B 没有任何 code-size 收益，反而会让当前 `240px` 图宽、`PFB_WIDTH=48` 横向 walk 下的 QOI/RLE alpha 热点出现 `+45% ~ +66%` 乃至数倍回退，因此不再保留 public 入口。

| 属性 | 值 |
|------|---|
| 默认值 | `0`（关闭 codec fast-draw） |
| HelloPerformance | 当前 shipped `1`（full row-band），历史 low-RAM 实验值 `2`（tail-row） |
| RAM 类型 | heap（峰值减小） |
| HP 节省 | `2` 相比 `1` 约 ~2304 B 峰值堆（QOI/RLE α 场景） |

**机制：**
`EGUI_CONFIG_FUNCTION_IMAGE_CODEC_FAST_DRAW` 现在统一承载 3 档策略：

- `0`：关闭 codec row-cache fast path
- `1`：启用 full row-band cache fast path
- `2`：启用 tail-row low-RAM fast path（隐含保留 row-cache 主路径）

其中标准 row-band 模式会让第一个 PFB tile 解码全图宽的当前行带并缓存，后续水平 tile 直接读缓存；tail 模式则让第一个 PFB tile 的像素从 scratch 流式解码并渲染，只把“后续 tile 尚未用到的尾列”写入 row cache。

在 240px 宽、48px PFB 步宽的场景下，第一 tile 占 48 列，尾部 192 列进入缓存，节省首 tile 的 `16×48×3 = 2304 B` 峰值堆分配。

`2026-04-04` 基于当前主线重新做完整 A/B 后，结论可以拆成两层理解：

- `0 -> 1`
  - 会额外带来约 `text +5108B`
  - 但如果关闭到 `0`，完整 `239` perf 场景里会有 `4` 个 `>=10%` 回退，全部集中在 internal tiled `QOI/RLE`
- `1 -> 2`
  - 会再额外带来 `HelloPerformance text +9976B`
  - 但能把 tail-row 路径重新打开，并在 `240px / PFB_WIDTH=48` 条件下再省下约 `2304B` 峰值堆
- 当前 shipped `1` 相比历史 low-RAM 实验值 `2`，在完整 `239` perf 场景里会有 `40` 个 `>=10%` 回退
...
因此这颗宏现在不能再按“单一布尔开关”理解：`1` 是当前 HelloPerformance shipped 的 full row-band 模式，`2` 是历史 low-RAM tail-row 候选值；如果后续需要重新平衡 codec heap 与压缩图吞吐，这仍是优先级很高的一颗重评估宏。
...
> `2026-04-06` 起，这个名字不再是当前框架 public 配置入口；当前主线已把它内收为实现私有常量，默认行为固定为 `1`。下面保留的是历史 A/B 记录，便于说明为什么它不再值得暴露成公共宏。

| 属性 | 值 |
|------|---|
| 默认值 | 当前实现内固定 `1`（预转换缓存） |
| HelloPerformance | 当前随实现内固定 `1`（历史 public 实验值 `0`） |
...
因此这颗宏当前默认继续保持 `1`。但它的收益也只有每个活动 QOI decode 实例 `~128B heap`，同时关闭后只能回收 `text -64B`，远低于当前“单颗 public 宏至少要值得 >1KB code-size 回收”的门槛；所以这个名字已不再保留为 public 宏入口，只保留历史 A/B 记录。
...
> `2026-04-06` 起，这个名字不再是当前框架 public 配置入口；当前主线已把它内收为 `src/image/egui_image_qoi.c` 里的实现私有常量，默认行为固定为 `0`。下面保留的是历史 A/B 记录，便于说明为什么它不再值得暴露成公共宏。
...
因此这颗宏当前默认继续保持 `0`。但它的收益只有每个活动实例 `~192B heap`，同时开启还会带来 `text +780B` 并在当前主线稳定打穿 `QOI RGB565` 主路径，远低于当前“单颗 public 宏至少要值得 >1KB code-size 回收”的门槛；所以这个名字已不再保留为 public 宏入口，只保留历史 A/B 记录。
...
| 属性 | 值 |
|------|---|
| 默认值 | 当前实现内固定 `0`（uint16_t，支持 ≤65535 行） |
| HelloPerformance | 当前随实现内固定 `0`（历史 public 实验值 `1`，uint8_t，限 ≤255 行） |
...
因此当前 shipped/default 继续记为 `0`；`1` 只保留为历史低 RAM 对比值，不再作为 `HelloPerformance` 的 active app-side override。由于收益只到每个解码实例 `~2~4B heap` 级别，同时完全拿不到 code-size 收益，这个名字已不再保留为 public 宏入口。
...
因此当前 shipped/default 继续记为 `1`；`0` 只保留为低优先级 low-RAM 条件实验值，不再作为 `HelloPerformance` 的 active app-side override。由于它只是在 `text +20B / bss -40B` 量级上做取舍，这个名字已不再保留为 public 宏入口。

---

### 9. `EGUI_CONFIG_IMAGE_TRANSFORM_EXTERNAL_ROW_PERSISTENT_CACHE_ENABLE`

**位置：** [src/canvas/egui_canvas_transform.c](../../../src/canvas/egui_canvas_transform.c)
...
因此当前 shipped/default 继续记为 `1`；`0` 只保留为低优先级 low-RAM 条件实验值，不再作为 `HelloPerformance` 的 active app-side override。由于它的收益也只有 `text -24B / bss -56B` 这一档，这个名字已不再保留为 public 宏入口。
...
### 11. `EGUI_CONFIG_IMAGE_RLE_EXTERNAL_WINDOW_PERSISTENT_CACHE_ENABLE`（已内收）
...
| HelloPerformance | 当前实现内固定 `1`（历史 public 实验值 `0`） |
...
- 历史实验值 `0` 现在只保留为旧 A/B 记录，不再是当前 app-side override
...
结论：这颗宏当前已从 public 配置面内收，默认 shipped 行为仍固定为 `1`。历史实验值 `0` 虽然能把 external RLE window 从 persistent BSS 切到 transient frame heap，并回收 `bss -1040B`，但会带来 `text +816B`，且 tiled / external RLE 仍有 `+2.7% ~ +6.2%` 的局部回退，因此现在只保留历史 A/B 记录。它与宏 10 的历史联动关系仍然成立：宏 10 决定 window 大小，宏 11 决定这块 window 历史上走 persistent BSS 还是 transient frame heap。
...
| 属性 | 值 |
|------|---|
| 默认值 | 当前实现内固定 `0`（uint16_t/uint32_t 字段） |
| HelloPerformance | 当前随实现内固定 `0`；历史 public 实验值为 `1`（uint8_t 紧凑） |
| RAM 类型 | data（静态缓存槽位） |
...
因此当前 shipped/default 继续记为 `0`；`1` 只保留为历史低 RAM 对比值。由于它的实际收益只有 `data -20B`，同时还会引入 `text +200B`，这个名字已不再保留为 public 宏入口。

---

### 13. `EGUI_CONFIG_FUNCTION_FONT_STD_FAST_DRAW`
...
| 属性 | 值 |
|------|---|
| 默认值 | 当前实现内固定 `0`（uint16_t） |
| HelloPerformance | 当前随实现内固定 `0`；历史 public 实验值为 `1`（uint8_t） |
...
因此当前 shipped/default 继续记为 `0`；`1` 只保留为历史低 RAM 对比值。当前四项验证只能证明它对静态 size、perf、渲染结果和单测都无差异；若要重新评估它的现实收益，需单独补运行期 heap 量化。由于现有证据还不足以证明它值得保留为当前 public 宏，这个名字已内收到实现内部。
...
|------|---|
| 默认值 | 当前实现内固定 `0` |
| HelloPerformance | 当前随实现内固定 `0`；历史 public 实验值为 `1` |
| RAM 类型 | heap + BSS |
...
因此当前 shipped/default 应记为 `0`；历史 `1` 不是 active app-side override，而是一个“多付 `text +488B` 和约 `164B` line-cache heap，换文本矩形/换行路径少量收益”的对比值。由于 code-size 收益没有达到保留 public 宏的门槛，这个名字已不再作为当前配置入口。
...
### 20. `EGUI_CONFIG_FUNCTION_FONT_TRANSFORM_FAST_DRAW`（prepare/layout 子能力）

**位置：** [src/canvas/egui_canvas_transform.c](../../../src/canvas/egui_canvas_transform.c)

| 属性 | 值 |
|------|---|
| 默认值 | 配置值 `1`，但实际受 `EGUI_CONFIG_FUNCTION_FONT_STD_FAST_DRAW>=1` 总门控 |
| HelloPerformance | `1`（同默认，**已保留**） |
| RAM 类型 | BSS |
| 大小 | ~60 B BSS |

**机制：**
预计算 text transform 的逆仿射矩阵（sin/cos × scale 混合）及旋转后包围盒，一帧内多个 PFB tile 渲染同一段文字只计算一次，其余 tile 直接复用。该子能力现在挂在 `EGUI_CONFIG_FUNCTION_FONT_STD_FAST_DRAW` 下：当 std-font fast draw 关闭时，它也会随之失效。禁用后每 tile 均需重新执行三角函数和矩阵求逆，当前 rotated-text 场景会出现稳定回归，60 B BSS 换取的性价比仍然很高，HelloPerformance 选择保留。

---

### 21. `EGUI_CONFIG_FUNCTION_FONT_TRANSFORM_FAST_DRAW`（dim cache 子能力）
...
**机制：**
`egui_canvas_draw_string_transform()` 内记录上一次调用时的 `(font, string)` 对及对应的宽高结果（4 个字段，共 12 B 量级）。同一帧内多个 tile 渲染同一字符串时，跳过 `get_str_size` 遍历。该 size cache 现在同样挂在 `EGUI_CONFIG_FUNCTION_FONT_STD_FAST_DRAW` 下。12 B BSS，性价比极高，HelloPerformance 保留。
...
|---|---|---|---|---|---|
| 1 | `EGUI_CONFIG_IMAGE_STD_ALPHA_OPAQUE_CACHE_SLOTS` | 当前实现内固定 **4**（历史 public 实验 **0**） | 当前随实现内固定 **4** | BSS | ~33 B（当前关闭仅回收 `text -156B, bss -40B`，且仍有 `8` 帧像素差，现已内化） |
...
| 3 | `EGUI_CONFIG_IMAGE_DECODE_OPAQUE_ALPHA_ROW_USE_ROW_CACHE` | 当前实现内固定 **0**（历史 public 实验 **1**） | 当前随实现内固定 **0** | heap+BSS | ~248 B（当前 `1` 仅额外拿到 `text -132B, bss -8B`，perf/runtime/unit 等价，现已内化） |
| 4 | `EGUI_CONFIG_FUNCTION_IMAGE_CODEC_FAST_DRAW` | 0 | 当前 shipped **1**（历史 low-RAM **2**） | heap峰值 | `1` 避免 internal tiled `QOI/RLE` 灾难性回退；`2` 相比 `1` 再省约 ~2304 B，但需额外 `text +9976B` |
| 5 | `EGUI_CONFIG_IMAGE_QOI_INDEX_RGB565_CACHE_ENABLE` | 当前实现内固定 **1**（历史 public 实验 **0**） | 当前随实现内固定 **1** | heap | ~128 B（当前关闭只回收 `text -64B`，现已内化） |
| 6 | `EGUI_CONFIG_IMAGE_QOI_COMPACT_RGB565_INDEX_ENABLE` | 当前实现内固定 **0**（历史 public 实验 **1**） | 当前随实现内固定 **0** | heap | ~192 B（历史 `1` 会带来 `text +780B` 且有 `10` 个 `>=10%` QOI 主路径回退，现已内化） |
| 7 | `EGUI_CONFIG_IMAGE_QOI_ROW_INDEX_8BIT_ENABLE` | 当前实现内固定 **0**（历史 public 实验 **1**） | 当前随实现内固定 **0** | heap | 历史 `1` 可回收 ~2~4 B / 解码实例（当前 `0 -> 1` 仅 `text +12B`，perf/runtime/unit 等价，现已内化） |
...
| 8 | `EGUI_CONFIG_IMAGE_STD_EXTERNAL_ROW_PERSISTENT_CACHE_ENABLE` | 当前实现内固定 **1**（历史 public 实验 **0**） | 当前随实现内固定 **1** | BSS | 历史 `0` 可回收 `bss -40B`（代价 `text +20B`，当前 perf/runtime/unit 等价，现已内化） |
| 9 | `EGUI_CONFIG_IMAGE_TRANSFORM_EXTERNAL_ROW_PERSISTENT_CACHE_ENABLE` | 当前实现内固定 **1**（历史 public 实验 **0**） | 当前随实现内固定 **1** | BSS | 历史 `0` 可回收 `text -24B, bss -56B`（当前 perf/runtime/unit 等价，现已内化） |
...
- 宏 20/21（同一颗 `EGUI_CONFIG_FUNCTION_FONT_TRANSFORM_FAST_DRAW` 的 prepare/layout 与 dim 子能力）现在是 `EGUI_CONFIG_FUNCTION_FONT_STD_FAST_DRAW` 的子能力；在总开关打开的前提下，它仍是文字旋转性能的关键，禁用会导致每 tile 重计算仿射/宽高，**不建议关闭**。
