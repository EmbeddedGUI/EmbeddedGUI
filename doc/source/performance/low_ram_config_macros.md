...
| 20 | `EGUI_CONFIG_FUNCTION_FONT_TRANSFORM_FAST_DRAW`（prepare/layout） | 1 | 1（保留）| BSS | 0（已开）|
| 21 | `EGUI_CONFIG_FUNCTION_FONT_TRANSFORM_FAST_DRAW`（dim） | 1 | 1（保留）| BSS | 0（已开）|
...
- 宏 20/21（同一颗 `EGUI_CONFIG_FUNCTION_FONT_TRANSFORM_FAST_DRAW` 的 prepare/layout 与 dim 子能力）是文字旋转性能的关键，禁用会导致每 tile 重计算仿射/宽高，**不建议关闭**。
