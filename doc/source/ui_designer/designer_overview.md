...
- 设计稿转换管道（HTML/JSX -> XML -> C）：已迁移到 `EmbeddedGUI_Designer` 独立仓库维护
...
该链路已迁移到 `EmbeddedGUI_Designer` 仓库，当前仓库仅保留 SDK 侧能力说明。

```
HTML/JSX 设计稿
    |
    v
在 EmbeddedGUI_Designer 仓库执行
...
同样地，资源面板自己的导入按钮现在也会先检查工程资源目录是否可用；未保存工程时会直接提示，而不是先让用户选完外部文件再报错。
...
# 3. 导出图标
python html2egui_helper.py export-icons --input design.html --app MyApp --size 24
