...
| Heap 峰值 | `6688B` | `QEMU_HEAP_MEASURE=1` 实测峰值 |
| Heap 空闲 current | `0B` | 当前默认低 RAM 方案下，尺寸相关 scratch 最终都会释放 |
| alloc/free | `5308 / 5308` | 完整录制流程结束后配平 |
...
- `heap peak=6688B` 是运行时峰值，不是 idle 常驻占用；当前默认示例结束后 `current heap` 会回到 `0B`。
...
- 当前正常 QEMU 构建口径是 `text=2135720`、`data=52`、`bss=2356`、`static RAM=2408`；buffered rotated-text 的 visible alpha8 ceiling 仍保持 `2560B`，两个文字热点继续维持在 `TEXT_ROTATE_BUFFERED=3334B`、`EXTERN_TEXT_ROTATE_BUFFERED=3544B`；继续往下到 `2304B` 反而会触发 packed4 fallback heap cliff，scene-local heap 回跳到 `5410B/5252B`。
...
- 当前默认 external raw-image shared row cache 也已收紧到 `2` 行上限：`EGUI_CONFIG_IMAGE_EXTERNAL_DATA_CACHE_MAX_BYTES=960`、`EGUI_CONFIG_IMAGE_EXTERNAL_ALPHA_CACHE_MAX_BYTES=480`。这不会改变压缩图主导的 whole-run headline，只是当前 headline 现在是 `6688B`；对应 `240px` 外部 RGB565+alpha 场景的 scene-local heap 从旧的 `2880B` 压到 `1440B`，resize 场景则是 `1536B`。
...
当前默认示例的 heap 以短生命周期 scratch 为主，峰值 `6688B`，空闲 `current` 回到 `0B`。
...
| --- | ---: | --- |
| `MASK_IMAGE_QOI_ROUND_RECT` | `6688B` | 当前已验证的单场景 heap 最大值之一；只在 codec alpha 场景使用 logical `96x8`，物理 `48x16` PFB 字节数不变 |
| `EXTERN_MASK_IMAGE_QOI_ROUND_RECT` | `6688B` | 同上，外部资源版本 |
| `MASK_IMAGE_QOI_CIRCLE` | `6688B` | 同上，当前 whole-run headline 会经过这类场景 |
| `EXTERN_MASK_IMAGE_QOI_CIRCLE` | `6688B` | 同上，外部资源版本 |
...
- 当前 whole-run heap peak 仍然由压缩图场景决定，而不是 text 场景；但 headline 已经从旧的 `_8` QOI alpha 路径转移到 plain masked QOI round-rect/circle 这组场景，当前是 `6688B`。
- 所有尺寸相关 scratch 最终都会回到 `0B` current heap；默认路径没有常驻 heap 大块缓存。
- 当前压缩图热点可以按 shipped 默认分成三档：plain masked QOI round-rect/circle 及其 extern 版本 `6688B`，tiled codec alpha `5008B / 4816B`，以及普通 QOI/RLE alpha `3760B / 3664B`。这些数值都来自当前 shipped 的按场景 logical `96x8` 路径；它不是被拒绝的“全场景 logical PFB probe”，而是只对 codec alpha 场景生效的定向 hint。
...
- 2026-03-29 又补做过 `1` 行 external raw-image row cache A/B：把 shared row cache 再压到 `480/240` 后，代表性 scene-local heap 会继续降到 `EXTERN_IMAGE_565_8 720B`、`EXTERN_IMAGE_RESIZE_565_8 816B`、`EXTERN_IMAGE_ROTATE_565_8 720B`，但 whole-run heap headline 仍然不变，还是被 codec 场景钉在当前的 `6688B`。
...
- 当前默认 heap 峰值 `6688B` 主要来自运行时 scratch，空间可回到 `0B`；当前最大的已验证单场景热点是 `MASK_IMAGE_QOI_ROUND_RECT` / `MASK_IMAGE_QOI_CIRCLE` 及其 extern 版本 `6688B`，随后是 `IMAGE_TILED_QOI_565_8 5008B`、`IMAGE_TILED_RLE_565_8 4816B`、`IMAGE_RLE_565_8` / `EXTERN_IMAGE_RLE_565_8 3760B`、`IMAGE_QOI_565_8` / `EXTERN_IMAGE_QOI_565_8` / `MASK_IMAGE_QOI_8_ROUND_RECT` / `EXTERN_MASK_IMAGE_QOI_8_ROUND_RECT 3664B`；若后续引入常驻 heap，必须明确记录 owner、lifetime、bytes。
