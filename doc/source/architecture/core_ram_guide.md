...
| Heap 峰值 | `9424B` | `QEMU_HEAP_MEASURE=1` 实测峰值 |
| Heap 空闲 current | `0B` | 当前默认低 RAM 方案下，尺寸相关 scratch 最终都会释放 |
| alloc/free | `4980 / 4980` | 完整录制流程结束后配平 |
...
当前默认示例的 heap 以短生命周期 scratch 为主，峰值 `9424B`，空闲 `current` 回到 `0B`。
...
| `MASK_IMAGE_QOI_8_ROUND_RECT` | `9424B` | 低 RAM codec tail-row cache + `208B` QOI 解码状态；当前 shipped `48px` walk 下，masked visible-span scratch 会借用已分配的 tail-row heap row，只有更宽的 clipped span 才会回退到额外 `144B` transient heap |
| `EXTERN_MASK_IMAGE_QOI_8_ROUND_RECT` | `9424B` | 同上，外部 masked QOI round-rect 场景 |
...
- 当前 whole-run heap peak 仍然由压缩 alpha 图片场景决定，而不是 text 场景；当前 headline 被 QOI alpha 场景钉在 `9424B`。
- 所有尺寸相关 scratch 最终都会回到 `0B` current heap；默认路径没有常驻 heap 大块缓存。
- 当前压缩图热点可以分成 shipped 与 fallback 两层：共享的 tail-row cache 仍是 `192` 列乘以 `16` 行、乘以 `RGB565 2B + alpha8 1B` 得到 `9216B`。在此基础上，unmasked QOI 场景是 `9216 + 208 = 9424B`；masked QOI 场景在当前 shipped `48px` walk 上也保持 `9424B`，因为可见段 scratch 会借用已分配的 tail-row heap row；只有 clipped span 超过该行容量时，才会临时回到 `9216 + 208 + 144 = 9568B`。RLE alpha 场景仍额外带 `144B` 可见段 scratch 和 `16B` 解码状态，因此保持 `9376B`。
...
- 当前默认 heap 峰值 `9424B` 主要来自运行时 scratch，空间可回到 `0B`；其中共享的 codec tail-row cache 是 `9216B`，unmasked QOI 场景额外的 `208B` 来自按帧解码状态，masked QOI 场景在当前 shipped walk 也保持这条线，因为可见段 scratch 会借用已分配的 tail-row heap row，只有更宽的 clipped span 才会额外临时增加 `144B`，而 RLE 场景仍是 `9216 + 144 + 16 = 9376B`；若后续引入常驻 heap，必须明确记录 owner、lifetime、bytes。
