...
2. egui_core_update_region_dirty()
   ...
   v
3. egui_check_need_refresh(core)
   |-- 检查 dirty 数组是否有非空区域
   v
4. egui_core_draw_view_group_pre_work(core)
   |-- compute_scroll(): 计算滚动偏移
   |-- calculate_layout(): 递归计算所有视图的 region_screen
   v
5. egui_polling_refresh_display(core)
   |-- 遍历每个脏区域
   |   +-- egui_core_draw_view_group(p_region_dirty)
   |       |-- 将脏区域划分为 PFB 大小的块
   |       |-- 对每个 PFB 块:
   |       |   +-- egui_canvas_init(..., core, ...): 初始化画布并绑定所属 core
...
for (y = 0; y < pfb_height_count; y++)
{
    for (x = 0; x < pfb_width_count; x++)
    {
        ...
        // 2. 初始化 Canvas，绑定 core 和 PFB 缓冲区
        egui_canvas_init(&core->canvas, core, pfb, &region);
