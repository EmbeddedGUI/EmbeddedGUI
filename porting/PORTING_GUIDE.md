...
> 新代码优先通过 `egui_setup_display()` 一次性完成 core、platform、display、touch、uicode 绑定；底层 port 负责准备 `egui_display_driver_t` / `egui_platform_t`，触摸通过 `egui_hal_touch_register(core, touch, &config)` 绑定到指定 core。
...
void egui_port_init(egui_core_t *core)
{
    ...
    egui_platform_register(core, &s_platform);
...
当前推荐把 LCD、Touch、Platform 相关单例准备都集中在 `egui_port_init()`，再由 `egui_setup_display()` 统一完成 core 初始化与注册：
...
- [ ] `egui_port_init()` 中已准备好 `egui_display_driver_t` / `egui_platform_t`，并能被 `egui_setup_display()` 或低层初始化流程正确绑定
...
- [ ] 主流程调用顺序正确：推荐 `egui_port_init(&core)` → `egui_setup_display(&core, &setup)` → `egui_polling_work(&core)`；旧单屏 port 至少保证 `egui_init()` 后再注册 platform / display / touch
