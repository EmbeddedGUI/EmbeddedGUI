...
7. 在主循环中执行 `egui_port_init()`，准备 `egui_display_setup_t`，调用 `egui_setup_display(&core, &setup)`，随后循环 `egui_polling_work(&core)`
...
`egui_port_init()` → `egui_setup_display(&core, &setup)` → 循环 `egui_polling_work(&core)`
...
- [ ] 主循环调用顺序正确：`egui_port_init()` → `egui_setup_display(&core, &setup)` → `egui_polling_work(&core)`
