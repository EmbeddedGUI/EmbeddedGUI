...
> 当前推荐架构：**Bus IO + HAL Driver + Bridge + Core**。
> 不再建议在 porting 中直接手写旧式 `egui_display_driver_t` / `egui_touch_driver_t` 逻辑。

## 1. 架构概览

EmbeddedGUI 当前采用四层结构：
...
└───────────────────────┬──────────────────────┘
                        │
              driver/bridge/egui_driver_bridge
                        │
        ┌───────────────┴────────────────┐
        │                                │
┌───────▼────────┐               ┌───────▼─────────┐
...
| Bridge | 是 | 把 HAL 驱动适配成 Core 的 Display/Touch 接口 |
...
- Core 层依然使用 `egui_display_driver_t` 注册显示驱动，但通常通过 `egui_display_driver_from_lcd()` 自动生成。
- Touch 也通过 `egui_touch_driver_bridge_register()` 完成桥接注册。
...
4. 在 `egui_port_init()` 中实例化 HAL 驱动，调用 `init()`，并通过 bridge 注册到 Core
...
struct egui_hal_lcd_driver {
    ...
    void (*wait_dma_complete)(egui_hal_lcd_driver_t *self);
...
如果你的平台支持 DMA 异步发送，建议实现 `wait_dma_complete()`。
...
void egui_port_init(void)
{
    ...
    egui_display_driver_register(egui_display_driver_from_lcd(&s_lcd_driver));
}
```
...
static void my_lcd_set_window(egui_hal_lcd_driver_t *self, int16_t x, int16_t y,
                              int16_t w, int16_t h)
{
    EGUI_UNUSED(self);
...
static void my_lcd_write_pixels(egui_hal_lcd_driver_t *self, const void *data, uint32_t len)
{
    EGUI_UNUSED(self);
...
如果 `write_pixels()` 走 DMA：

- `write_pixels()` 中启动 DMA 发送
- `wait_dma_complete()` 中等待完成
...
当前 Core 最终仍以单点事件为主，bridge 默认会把第一触点作为主触点上报给 Core。
...
如果平台有 DMA2D / PXP / GPU，可在 bridge 适配后的显示层使用 `fill_rect`、`blit`、`blend` 一类能力加速。
...
└── your_platform/
    ...
    │   ├── egui_port_mcu.c            # HAL Driver + Bridge + Platform 注册
    │   └── port_main.c                # 主循环入口
    ├── app_egui_config.h
    └── build.mk
```
...
- [ ] `egui_port_init()` 中已注册 LCD bridge 和 Platform Driver
- [ ] 如启用触摸，已调用 `egui_touch_driver_bridge_register()`
- [ ] LCD 驱动的 `set_window()` / `write_pixels()` 能正确输出像素
